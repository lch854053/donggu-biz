...
사업자 조회는 유연한 입력 스테이션과 280px 기준 안내를 나란히 배치한다. 상권 분석은 300px 선택·상세 레일과 최소 420px 지도의 두 구역으로 구성하며 지도 높이는 최소 650px이다. 통계는 전체 업소, 현재 조건, 행정동, 최다 업종, 주요상권 포함을 같은 폭으로 놓는 다섯 칸 스트립이며, 결과는 가로 스크롤 가능한 고밀도 표로 표현한다.

1120px 이하에서는 선택·상세 레일을 270px로 줄이고 지도의 최소 폭을 유지한다. 780px 이하에서는 모든 업무 구역이 세로 흐름으로 바뀌고, 다섯 지표는 두 칸씩 두 행을 만든 뒤 마지막 주요상권 포함 지표가 전체 폭을 차지한다. 행정동·주요상권 선택과 선택 상권 상세가 지도보다 먼저 나오며 지도는 56vh 높이를 확보한다. 기준 안내와 출처 태그처럼 부차적인 정보는 좁은 화면에서 숨기고 핵심 입력, 결과, 선택과 분석을 보존한다. 520px 이하에서는 버튼이 두 열로 늘어나고 결과 표의 과세 유형 열이 생략된다.
...
- **Zone Selector:** 상권 레일에는 행정동과 주요상권 선택기만 둔다. 전체 지역 또는 개별 경계를 선택하며, 경계 데이터가 없으면 주요상권 선택기를 비활성화한다.
...
- **Behavior:** 주요상권 경계는 항상 표시한다. 폴리곤을 클릭하면 선택기와 왼쪽 상권 상세가 동기화되고 32px 지도 여백, 최대 줌 16으로 경계에 맞춘다. 점포 표식은 상호, 업종과 주소 팝업만 제공한다.
...
### Zone Analysis Panel
- **Style:** 주요상권 선택기 바로 아래에서 이름은 15px 굵은 제목, 항목명은 12px 보조 텍스트, 값은 12px 고정폭 우측 정렬을 사용한다. 상위 소분류는 최대 10개 막대로 이어진다.
- **Overview State:** 상권을 선택하지 않으면 선택 후 확인 가능한 정보를 한 문장으로 안내한다.
- **Selected State:** 선택한 상권 이름, 면적, 전체 점포, 현재 조건 점포와 상위 10개 업종 소분류 막대를 보여 준다.

### Data Caveat
- **Style:** 12px 황색 제목과 12px 보조 본문으로 주요상권 경계 범위와 상가정보의 한계를 설명한다. 선택 상권 상세 아래 20px 여백, 14px 상단 패딩과 1px 구조선을 사용한다.
- **Content:** VWorld 및 수동 등록 주요상권이 동구 전체 상권을 의미하지 않으며, 상가정보에는 매출·임대료·유동인구·사업자등록번호가 없고 표시 건수가 수익성이나 영업 상태를 의미하지 않는다고 명시한다.
- **Responsive:** 모든 화면 크기에서 선택 상권 상세 바로 아래 순서를 유지한다.
