...
- **Outline Variant:** 상권 상세 분석 하위 탭의 별도 지도에는 선택 상권 경계와 경계 중심점이 내부에 있는 건물 윤곽만 표시한다. manifest와 교차하는 cell만 지연 로드하며, PNU를 우선하고 좌표·같은 필지·거리 fallback으로 연결한 우세 업종을 색칠한다. 한 건물에 여러 업소가 연결되면 업소 수가 가장 많은 업종을 사용하고, 업종 데이터가 없는 도형에는 hover 정보를 제공하지 않는다. 업종 색 범례는 별도 지도 좌측 하단의 반투명 박스로 표시한다.
...
- **Building Outline State:** 상권 상세 분석 하위 탭의 좌측 레일에 경계·업소·연결 점포 수와 단색 전환을 둔다. 업종 분류는 기본으로 켜고, 업종 색 범례는 별도 지도 좌측 하단에 둔다. 건물 데이터가 없으면 빈 상태로, 점포를 연결하지 못한 도형은 `점포 미연결`로 설명한다.
