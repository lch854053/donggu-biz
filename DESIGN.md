...
- **National Pension Variant:** 국민연금 조회에서는 스테이션 제목 아래 12px 보조 텍스트 세 줄(출처·조회 지역·유의사항)이 같은 역할을 한다. 사업자등록번호가 앞 6자리까지만, 주소가 건물번호 없이 도로명까지만 제공되어 개별 사업자를 특정할 수 없다는 점, 도로명별 집계가 건물 단위 분포가 아니라는 점, 목록 API에 가입자 수가 없어 모든 수치가 사업장 수라는 점, 미가입 사업장이 빠져 지역 전체 사업체 수와 다르다는 점을 명시한다.
