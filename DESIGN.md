...
1120px 이하에서는 분석 레일이 지도 아래 전체 폭으로 이동해 24px 간격의 두 열이 된다. 주요상권 분석은 유연한 첫째 열, 해석 유의사항은 최소 240px의 둘째 열에 놓이며 둘 사이의 세로 구분선은 제거한다. 780px 이하에서는 모든 업무 구역이 세로 흐름으로 바뀌고, 다섯 지표는 두 칸씩 두 행을 만든 뒤 마지막 주요상권 포함 지표가 전체 폭을 차지한다. 지도는 56vh 높이를 확보하고 분석 레일은 주요상권 분석 다음에 상단 구분선을 둔 해석 유의사항이 이어지는 수직 배열로 돌아간다. 기준 안내와 출처 태그처럼 부차적인 정보는 좁은 화면에서 숨기고 핵심 입력, 결과, 필터와 분석을 보존한다. 520px 이하에서는 버튼과 필터가 두 열로 늘어나고 결과 표의 과세 유형 열이 생략된다.
...
### Map Workspace
- **Style:** 좌우 레일과 중앙 지도를 한 12px 패널 안에 결합한다. Leaflet 컨트롤, 팝업과 클러스터도 같은 청흑색 표면과 청색 신호를 사용한다.
- **Behavior:** 주요상권 폴리곤을 클릭하면 선택기와 분석 패널이 동기화되고 32px 지도 여백, 최대 줌 16으로 경계에 맞춘다. 점포 표식은 상호, 업종과 주소 팝업만 제공한다.
...
### Zone Analysis Panel
- **Style:** 오른쪽 분석 레일의 첫 섹션이다. 제목 옆 고정폭 배지는 등록 경계 수를 표시하고, 이름은 15px 굵은 제목, 항목명은 12px 보조 텍스트, 값은 12px 고정폭 우측 정렬을 사용한다. 해석 유의사항은 데스크톱에서 20px 아래에 1px 상단 구분선과 함께 이어진다.
- **Overview State:** 상권을 선택하지 않으면 등록 경계, 경계 내부·외부 업소 수와 경계 갱신일을 네 행으로 보여 준다.
- **Selected State:** 선택한 상권 이름, 면적, 전체 점포, 현재 조건 점포와 상위 세 업종 막대를 보여 준다. 경계 데이터가 없으면 점포·업종 필터만 제공한다는 설명 상태로 바뀐다.

### Data Caveat
- **Style:** 12px 황색 제목과 12px 보조 본문으로 VWorld 경계 범위와 상가정보의 한계를 설명한다. 기본 데스크톱에서는 주요상권 분석 아래 20px 여백, 14px 상단 패딩과 1px 구조선을 사용한다.
- **Content:** VWorld 주요상권이 동구 전체 상권을 의미하지 않으며, 상가정보에는 매출·임대료·유동인구·사업자등록번호가 없고 표시 건수가 수익성이나 영업 상태를 의미하지 않는다고 명시한다.
- **Responsive:** 1120px 이하의 두 열 레이아웃에서는 여백과 구분선을 제거해 독립된 둘째 열이 되고, 780px 이하에서는 다시 분석 아래로 내려가 상단 구분선을 회복한다.
