...
- **Zone Selector:** 행정동과 업종 선택기 사이에 주요상권 선택기를 둔다. 전체 지역 또는 개별 경계를 선택하며, 경계 데이터가 없으면 비활성화한다.
...
### Data Caveat
- **Style:** 12px 황색 제목과 12px 보조 본문으로 주요상권 경계 범위와 상가정보의 한계를 설명한다. 기본 데스크톱에서는 주요상권 분석 아래 20px 여백, 14px 상단 패딩과 1px 구조선을 사용한다.
