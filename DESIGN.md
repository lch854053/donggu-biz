...
- **Behavior:** 주요상권 경계는 항상 표시한다. 폴리곤을 클릭하면 행정동 선택을 초기화하고 주요상권 선택기와 왼쪽 지역 상세를 동기화한 뒤 32px 지도 여백, 최대 줌 16으로 경계에 맞춘다. 행정동 선택 시 점포·클러스터 마커를 표시하고, 주요상권 선택 시 이를 숨긴 뒤 선택 경계 안의 건물 윤곽을 지연 로드한다. 업종 분류는 기본으로 켜며, 업종 데이터가 있는 건물 hover에서 연결 업소명·업종·주소를 보여 준다.
...
- **Outline Variant:** 주요상권 선택 시 같은 상권 지도에 선택 경계와 경계 중심점이 내부에 있는 건물 윤곽만 표시한다. manifest와 교차하는 cell만 지연 로드하며, PNU를 우선하고 좌표·같은 필지·거리 fallback으로 연결한 우세 업종을 색칠한다. 한 건물에 여러 업소가 연결되면 업소 수가 가장 많은 업종을 사용하고, 업종 데이터가 없는 도형에는 hover 정보를 제공하지 않는다. 지도 이동은 선택 경계 주변으로 제한한다.
