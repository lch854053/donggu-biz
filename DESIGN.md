...
밀도는 높지만 역할은 명확하다. 산세리프 한글은 설명과 조작을 담당하고, 고정폭 숫자는 건수·등록번호·면적·비율처럼 비교해야 하는 데이터를 정렬한다. 의미색은 상태 배지와 경고에 집중하되, 지도 전체 지역 상태에서는 주황색 주요상권 경계, 상권 선택 상태에서는 황색 비선택 경계와 청색 선택 경계로 공간 상태를 구분한다. 지도, 표와 상권 분석 패널은 동일한 표면·선·타입 체계를 공유한다.

**Key Characteristics:**
- 무광 청흑색 표면이 층을 이루는 어두운 업무 환경
- 청색 동작·선택과 주황색·황색 지도 경계를 굵기·채움 차이로 보강하는 체계
...
- **주의·경계 황색:** 휴업과 해석 유의사항을 표시하고, 지도에서는 주요상권 선택 중 나머지 상권을 2px 선과 낮은 채움으로 구획한다.
- **전체 경계 주황색:** 주요상권을 선택하지 않은 지도에서 모든 상권을 3px 선과 12% 채움으로 선명하게 구획한다.
...
- **Polygon States:** 전체 지역 상태의 경계는 주황색 3px 선, 100% 선 불투명도와 12% 채움이며 hover 시 4px·20%로 높인다. 주요상권 선택 중 나머지 경계는 황색 2px 선, 82% 선 불투명도와 6% 채움을 사용하고 hover 시 3px·12%로 높인다. selected는 신호 청색 3px 선, 100% 선 불투명도와 관제 청색 18% 채움으로 바뀐다.
