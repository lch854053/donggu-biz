...
사업자 상태조회는 유연한 입력 스테이션과 280px 기준 안내를 나란히 배치한다. 국민연금 가입 사업장 조회는 조건이 일곱 가지로 늘어 안내 열을 걷어내고 입력 스테이션이 작업면 전체를 쓰며, 조건은 220px 최소 폭으로 자동 배치되는 격자에 놓는다. 출처·조회 지역·유의사항은 옆 열이 아니라 스테이션 제목 아래 세 줄로 붙인다. 상권 분석은 300px 선택·상세 레일과 최소 420px 지도의 두 구역으로 구성하며 지도 높이는 최소 650px이다. 통계는 전체 업소, 현재 조건, 행정동, 최다 업종, 주요상권 포함을 같은 폭으로 놓는 다섯 칸 스트립이며, 결과는 가로 스크롤 가능한 고밀도 표로 표현한다.
...
- **Sub Navigation:** 사업자 조회 안의 세 조회 종류(사업자 상태조회·사업자등록정보 진위확인·국민연금 가입 사업장 조회)는 38px 높이의 서류철 탭이다. 비선택 탭은 경계 없는 흐린 텍스트이고, 선택 탭은 1px 구조선과 패널 표면을 이어 붙인 상단 7px 곡률 형태에 신호 청색 텍스트를 쓴다. 상위 탭과 위계를 구분하기 위해 하단선 대신 표면 연결로 활성 상태를 표시한다.
...
- **Basis Line:** 국민연금 조회는 실시간 호출이 아니라 월 1회 갱신하는 스냅샷을 읽는다. 자료가 언제 것인지를 유의사항 아래 12px 고정폭 흐린 텍스트 한 줄로 적어, 유의사항보다 한 단계 낮은 무게로 항상 보이게 한다. 사업자등록번호가 앞 6자리까지만, 주소가 건물번호 없이 도로명까지만 제공되어 개별 사업자를 특정할 수 없다는 점, 도로명별 집계가 건물 단위 분포가 아니라는 점, 목록 API에 가입자 수가 없어 모든 수치가 사업장 수라는 점, 미가입 사업장이 빠져 지역 전체 사업체 수와 다르다는 점을 명시한다.
