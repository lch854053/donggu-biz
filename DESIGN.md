...
- **Outline Variant:** 상권 분석 지도는 선택한 주요상권 경계와 경계 중심점이 내부에 있는 건물 윤곽만 표시한다. manifest와 교차하는 cell만 지연 로드하며, PNU 또는 점포 좌표로 연결한 우세 업종을 색칠한다. 지도 이동은 선택 경계 주변으로 제한한다.
...
- **Building Outline State:** 선택한 상권은 별도 상단 탭에서 다시 확인할 수 있으며, 300px 상세 레일에 경계·업소·연결 점포 수, 업종 범례와 단색 전환을 둔다. 건물 데이터가 없거나 연결되지 않은 도형은 빈 상태 또는 `업종 미확인`으로 설명한다.
