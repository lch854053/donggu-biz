...
- **Sub Navigation:** 사업자 조회 안의 세 조회 종류(사업자 상태조회·사업자등록정보 진위확인·4대보험 사업장 조회)와 상가·상권 조회 안의 네 조회 방식(상가 조회·상권 지도·상권 분석·폐업 분석)은 38px 높이의 서류철 탭이다. 비선택 탭은 경계 없는 흐린 텍스트이고, 선택 탭은 1px 구조선과 패널 표면을 이어 붙인 상단 7px 곡률 형태에 신호 청색 텍스트를 쓴다. 상위 탭과 위계를 구분하기 위해 하단선 대신 표면 연결로 활성 상태를 표시한다.
...
- **Closure Analysis:** 폐업 분석 하위 탭은 조건 격자, 세 칸 요약 타일, 연도별 막대 목록, 폐업률 표, 해석 기준 상자를 세로로 쌓는다. 요약 타일은 190px 최소 폭으로 자동 배치하고 수치는 고정폭 24px로 놓는다. 연도별 분포는 상권 상세와 같은 4px 트랙 막대를 재사용해 최근 연도를 위에 둔다. 폐업률 표는 행정동별·업종별을 같은 표에서 전환하며, 폐업률 열만 굵게 강조한다. 해석 기준 상자는 주의 황색 제목에 보조 본문으로 두 문단을 담고, 행정동을 확정하지 못한 건이 없으면 둘째 문단을 감춘다. 휴업을 고르면 폐업일자가 없으므로 연도 선택을 잠근다.
