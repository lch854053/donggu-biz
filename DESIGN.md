...
- **Behavior:** 주요상권 경계는 항상 표시한다. 폴리곤을 클릭하면 행정동 선택을 초기화하고 주요상권 선택기와 왼쪽 지역 상세를 동기화한 뒤 32px 지도 여백, 최대 줌 16으로 경계에 맞춘다. 행정동 또는 주요상권 선택 시 점포·클러스터 마커를 표시하고, 폐업 업소 표시를 켜면 같은 선택 범위의 폐업 이력을 경고 적색 마커로 겹쳐 놓는다. 상권 분석 하위 탭에서는 선택 상권의 별도 지도에 건물 윤곽을 지연 로드하고, 주요상권을 선택한 경우에만 3D 매스 모드를 제공하며, 선택 후에도 지도 이동은 제한하지 않는다.
...
- **Outline Variant:** 상권 분석 하위 탭의 별도 지도에는 외곽 상권 경계선과 배경 지도를 두지 않고 경계 내부에 있는 건물 윤곽만 표시한다. 주요상권을 선택하면 같은 도형을 3D 매스로 전환할 수 있지만 행정동·주소 반경 선택에서는 3D 모드 컨트롤과 매스를 숨긴다. manifest와 교차하는 cell만 지연 로드하며, PNU를 우선하고 좌표·같은 필지·거리 fallback으로 연결한 우세 업종을 색칠한다. 한 건물에 여러 업소가 연결되면 업소 수가 가장 많은 업종을 사용하고, 업종 데이터가 없는 도형에는 hover 정보를 제공하지 않는다. 업종 색 범례는 별도 지도 좌측 하단의 반투명 박스로 표시한다.
...
- **Statistics Tab:** 통계는 사업자 조회·상가·상권 조회와 동급 상단 탭이다. 화면 제목 아래에 주제 묶음을 세로로 쌓고, 주제마다 소제목·출처 줄·그래프를 갖는다. 소제목은 17px 굵은 본문 잉크이고 그 아래 출처 줄은 12px 보조 잉크이며, 묶음 머리는 1px 구조선으로 본문과 나눈다. 한 주제 안에서는 전폭 선 그래프 한 장과 2열 막대 그래프 네 장을 놓는다. 조건 입력·요약 타일·해석 상자는 두지 않는다.

- **Chart Marks:** 모든 그래프는 계열이 하나이므로 범례를 두지 않고 제목이 무엇을 그렸는지 말한다. 데이터 잉크는 동작색 하나만 쓰고, 값·축·라벨 문자는 본문 잉크를 쓴다. 선은 2px에 둥근 마디, 점은 반지름 4px에 배경색 2px 링을 두른다. 막대는 가로 방향으로 최대 24px 두께이고 남는 자리는 여백으로 두며, 값 쪽 끝만 4px 둥글게 하고 기준선 쪽은 각지게 둔다. 격자선과 축선은 구조선 색의 1px 실선이며 파선을 쓰지 않는다. 막대는 값을 끝에 직접 적고, 선 그래프는 마지막 값 하나만 적어 숫자가 모든 점에 붙지 않게 한다. 축 눈금은 1·2·2.5·5의 10의 거듭제곱 배수로만 올린다.

- **Chart Reading:** 그래프는 값을 읽는 유일한 통로가 아니다. 막대는 값을 끝에 직접 적어 표 없이도 읽히고, 연도별 추이는 `표로 보기`를 접어 두어 같은 값을 표로도 읽을 수 있다. 막대와 연도는 마우스 오버와 키보드 포커스에서 같은 말풍선을 띄운다. 말풍선은 값을 굵게 앞세우고 이름을 뒤에 둔다. 적중 영역은 막대 한 줄 전체와 선 그래프의 연도 구간 전체로 잡아 2px 도형을 정확히 겨냥하지 않아도 되게 한다.

- **Building Outline State:** 상권 분석 하위 탭의 좌측 레일에 선택 상권·업소·연결 점포 수와 2D/3D 전환을 둔다. 3D 전환은 주요상권에서만 노출하고, 업종 분류는 기본으로 켜며 업종 색 범례는 별도 지도 좌측 하단에 둔다. 건물 데이터가 없으면 빈 상태로, 점포를 연결하지 못한 도형은 `점포 미연결`로 설명한다.
