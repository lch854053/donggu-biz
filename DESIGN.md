...
밀도는 높지만 역할은 명확하다. 산세리프 한글은 설명과 조작을 담당하고, 고정폭 숫자는 건수·등록번호·면적·비율처럼 비교해야 하는 데이터를 정렬한다. 의미색은 상태 배지와 경고에 집중하되, 지도에서는 황색 주요상권 경계와 청색 선택 경계로 공간 상태를 구분한다. 지도, 표와 건물 윤곽 레이어는 동일한 표면·선·타입 체계를 공유한다.
...
### Secondary
- **확인 녹색:** 조회 성공, 계속사업자와 결과 내보내기를 나타낸다. 연한 전경색과 짙은 녹색 배경을 항상 함께 사용한다.
...
업종 분류 지도는 범주를 비교해야 하므로 낮은 채도의 보조 색상 팔레트를 예외적으로 사용한다. 업종 미확인은 중립 회색으로 두고, 업종 분류를 끄면 모든 도형을 하나의 중립 색으로 되돌린다.

**The Semantic Pair Rule.** 사업자 상태의 성공·주의·경고는 전경색과 전용 어두운 배경을 쌍으로 사용한다. 지도 경계는 색에 더해 선 굵기, 채움 농도, 선택기와 선택 레일을 함께 바꿔 상태를 전달한다.
...
사업자 상태조회는 유연한 입력 스테이션과 280px 기준 안내를 나란히 배치한다. 4대보험 사업장 조회는 국민연금·고용·산재 조건이 늘어 안내 열을 걷어내고 입력 스테이션이 작업면 전체를 쓰며, 조건은 220px 최소 폭으로 자동 배치되는 격자에 놓는다. 출처·조회 지역·자료 범위·유의사항은 옆 열이 아니라 스테이션 제목 아래에 붙인다. 소상공인 지도는 300px 선택·상세 레일과 최소 420px 지도의 두 구역으로 구성하며 지도 높이는 최소 650px이다. 주요상권을 선택하면 지도의 점포·클러스터 마커를 유지하고, 상권 상세 분석 하위 탭에서 300px 상세 레일과 최소 420px 건물 윤곽 지도를 별도로 제공한다. 통계는 전체 업소, 현재 조건, 행정동, 최다 업종, 주요상권 포함을 같은 폭으로 놓는 다섯 칸 스트립이며, 결과는 가로 스크롤 가능한 고밀도 표로 표현한다.

1120px 이하에서는 선택·상세 레일을 270px로 줄이고 지도의 최소 폭을 유지한다. 780px 이하에서는 모든 업무 구역이 세로 흐름으로 바뀌고, 다섯 지표는 두 칸씩 두 행을 만든 뒤 마지막 주요상권 포함 지표가 전체 폭을 차지한다. 행정동·주요상권 선택과 선택 지역 상세가 지도보다 먼저 나오며 지도는 56vh 높이를 확보한다. 기준 안내와 출처 태그처럼 부차적인 정보는 좁은 화면에서 숨기고 핵심 입력, 결과, 선택과 상권 상세 분석을 보존한다. 520px 이하에서는 버튼이 두 열로 늘어나고 결과 표의 과세 유형 열이 생략된다.

**The Parallel-Then-Sequential Rule.** 데스크톱은 조건·지도·상권 상세를 같은 서비스 안에서 제공하고, 모바일은 같은 순서를 위에서 아래로 풀어낸다. 좁은 화면에서 데스크톱 그리드를 축소하지 않는다.
...
### Navigation
- **Style:** 헤더 아래의 두 서비스(사업자 조회·소상공인 조회)는 48px 높이의 동급 탭이다. 기본 상태는 보조 텍스트, hover는 밝은 텍스트와 미세한 표면 변화, active는 신호 청색 텍스트와 2px 하단선이다. 모바일에서는 두 탭이 같은 폭을 차지한다.
- **Sub Navigation:** 사업자 조회 안의 세 조회 종류(사업자 상태조회·사업자등록정보 진위확인·4대보험 사업장 조회)와 소상공인 조회 안의 세 조회 방식(지도로 조회·상권 상세 분석·표로 조회)은 38px 높이의 서류철 탭이다. 비선택 탭은 경계 없는 흐린 텍스트이고, 선택 탭은 1px 구조선과 패널 표면을 이어 붙인 상단 7px 곡률 형태에 신호 청색 텍스트를 쓴다. 상위 탭과 위계를 구분하기 위해 하단선 대신 표면 연결로 활성 상태를 표시한다.
...
### Map Workspace
- **Style:** 왼쪽 선택·상세 레일과 지도를 한 12px 패널 안에 결합한다. Leaflet 컨트롤, 팝업·툴팁과 클러스터도 같은 청흑색 표면과 청색 신호를 사용한다.
- **Behavior:** 주요상권 경계는 항상 표시한다. 폴리곤을 클릭하면 행정동 선택을 초기화하고 주요상권 선택기와 왼쪽 지역 상세를 동기화한 뒤 32px 지도 여백, 최대 줌 16으로 경계에 맞춘다. 행정동 또는 주요상권 선택 시 점포·클러스터 마커를 표시한다. 상권 상세 분석 하위 탭에서는 선택 상권의 별도 지도에 건물 윤곽을 지연 로드하고, 선택 후에도 지도 이동은 제한하지 않는다.
...
- **Outline Variant:** 상권 상세 분석 하위 탭의 별도 지도에는 외곽 상권 경계선과 배경 지도를 두지 않고 경계 내부에 있는 건물 윤곽만 표시한다. manifest와 교차하는 cell만 지연 로드하며, PNU를 우선하고 좌표·같은 필지·거리 fallback으로 연결한 우세 업종을 색칠한다. 한 건물에 여러 업소가 연결되면 업소 수가 가장 많은 업종을 사용하고, 업종 데이터가 없는 도형에는 hover 정보를 제공하지 않는다. 업종 색 범례는 별도 지도 좌측 하단의 반투명 박스로 표시한다.
...
- **Building Outline State:** 상권 상세 분석 하위 탭의 좌측 레일에 선택 상권·업소·연결 점포 수와 단색 전환을 둔다. 업종 분류는 기본으로 켜고, 업종 색 범례는 별도 지도 좌측 하단에 둔다. 건물 데이터가 없으면 빈 상태로, 점포를 연결하지 못한 도형은 `점포 미연결`로 설명한다.
...
- **Four Insurance Variant:** 4대보험 조회에서는 스테이션 제목 아래 12px 보조 텍스트로 `출처 : 국민연금공단·근로복지공단 자료 (연 1회 갱신)`, 조회 지역과 자료 범위·건강보험 미제공을 표시한다.
- **Business Number Group Line:** 같은 사업자등록번호의 고용·산재 원본은 한 행에 묶고, 상세에서는 사업장관리번호를 소제목으로 삼아 주소·보험 구분·수치·성립일자·사업 구분·업종을 한 묶음으로 보여 준다. XLSX에는 관리번호별 수치를 보존하며, 메인 표는 회사 단위로 읽힌다.
...
- **Do** 넓은 화면의 병렬 업무 구조를 모바일에서 입력→결과 또는 필터→지도→건물 윤곽의 세로 순서로 보존한다.
