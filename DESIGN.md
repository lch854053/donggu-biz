...
사업자 상태조회는 유연한 입력 스테이션과 280px 기준 안내를 나란히 배치한다. 4대보험 사업장 조회는 국민연금·고용·산재 조건이 늘어 안내 열을 걷어내고 입력 스테이션이 작업면 전체를 쓰며, 조건은 220px 최소 폭으로 자동 배치되는 격자에 놓는다. 출처·조회 지역·자료 범위·유의사항은 옆 열이 아니라 스테이션 제목 아래에 붙인다. 상권 분석은 300px 선택·상세 레일과 최소 420px 지도의 두 구역으로 구성하며 지도 높이는 최소 650px이다. 통계는 전체 업소, 현재 조건, 행정동, 최다 업종, 주요상권 포함을 같은 폭으로 놓는 다섯 칸 스트립이며, 결과는 가로 스크롤 가능한 고밀도 표로 표현한다.
...
### Navigation
- **Style:** 헤더 아래의 세 서비스(사업자 조회·소상공인 조회·상권 분석)는 48px 높이의 동급 탭이다. 기본 상태는 보조 텍스트, hover는 밝은 텍스트와 미세한 표면 변화, active는 신호 청색 텍스트와 2px 하단선이다. 모바일에서는 세 탭이 같은 폭을 차지한다.
...
- **Outline Variant:** 상권 분석 지도는 선택한 주요상권 경계와 경계 중심점이 내부에 있는 건물 윤곽만 표시한다. manifest와 교차하는 cell만 지연 로드하며, PNU를 우선하고 좌표·같은 필지·거리 fallback으로 연결한 우세 업종을 색칠한다. 지도 이동은 선택 경계 주변으로 제한한다.
...
- **Building Outline State:** 선택한 상권은 별도 상단 탭에서 다시 확인할 수 있으며, 300px 상세 레일에 경계·업소·연결 점포 수, 업종 범례와 단색 전환을 둔다. 건물 데이터가 없으면 빈 상태로, 점포를 연결하지 못한 도형은 `점포 미연결`로 설명한다.
