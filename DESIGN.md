...
- **Sub Navigation:** 사업자 조회 안의 두 조회 종류(사업자 상태조회·국민연금 가입 사업장 조회)는 38px 높이의 서류철 탭이다. 비선택 탭은 경계 없는 흐린 텍스트이고, 선택 탭은 1px 구조선과 패널 표면을 이어 붙인 상단 7px 곡률 형태에 신호 청색 텍스트를 쓴다. 상위 탭과 위계를 구분하기 위해 하단선 대신 표면 연결로 활성 상태를 표시한다.
