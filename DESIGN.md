...
사업자 상태조회는 유연한 입력 스테이션과 280px 기준 안내를 나란히 배치한다. 4대보험 사업장 조회는 국민연금·고용·산재 조건이 늘어 안내 열을 걷어내고 입력 스테이션이 작업면 전체를 쓰며, 조건은 220px 최소 폭으로 자동 배치되는 격자에 놓는다. 출처·조회 지역·자료 범위·유의사항은 옆 열이 아니라 스테이션 제목 아래에 붙인다. 상권 지도는 300px 선택·상세 레일과 최소 420px 지도의 두 구역으로 구성하며 지도 높이는 최소 650px이다. 주요상권을 선택하면 지도의 점포·클러스터 마커를 유지하고, 상권 분석 하위 탭에서 300px 상세 레일과 최소 420px 건물 윤곽 지도를 별도로 제공한다. 통계는 전체 업소, 현재 조건, 행정동, 최다 업종, 주요상권 포함을 같은 폭으로 놓는 다섯 칸 스트립이며, 결과는 가로 스크롤 가능한 고밀도 표로 표현한다.

1120px 이하에서는 선택·상세 레일을 270px로 줄이고 지도의 최소 폭을 유지한다. 780px 이하에서는 모든 업무 구역이 세로 흐름으로 바뀌고, 다섯 지표는 두 칸씩 두 행을 만든 뒤 마지막 주요상권 포함 지표가 전체 폭을 차지한다. 행정동·주요상권 선택과 선택 지역 상세가 지도보다 먼저 나오며 지도는 56vh 높이를 확보한다. 기준 안내와 출처 태그처럼 부차적인 정보는 좁은 화면에서 숨기고 핵심 입력, 결과, 선택과 상권 분석을 보존한다. 520px 이하에서는 버튼이 두 열로 늘어나고 결과 표의 과세 유형 열이 생략된다.
...
### Navigation
- **Style:** 헤더 아래의 두 서비스(사업자 조회·상가·상권 조회)는 48px 높이의 동급 탭이다. 기본 상태는 보조 텍스트, hover는 밝은 텍스트와 미세한 표면 변화, active는 신호 청색 텍스트와 2px 하단선이다. 모바일에서는 두 탭이 같은 폭을 차지한다.
- **Sub Navigation:** 사업자 조회 안의 세 조회 종류(사업자 상태조회·사업자등록정보 진위확인·4대보험 사업장 조회)와 상가·상권 조회 안의 세 조회 방식(상가 조회·상권 지도·상권 분석)은 38px 높이의 서류철 탭이다. 비선택 탭은 경계 없는 흐린 텍스트이고, 선택 탭은 1px 구조선과 패널 표면을 이어 붙인 상단 7px 곡률 형태에 신호 청색 텍스트를 쓴다. 상위 탭과 위계를 구분하기 위해 하단선 대신 표면 연결로 활성 상태를 표시한다.
...
- **Behavior:** 주요상권 경계는 항상 표시한다. 폴리곤을 클릭하면 행정동 선택을 초기화하고 주요상권 선택기와 왼쪽 지역 상세를 동기화한 뒤 32px 지도 여백, 최대 줌 16으로 경계에 맞춘다. 행정동 또는 주요상권 선택 시 점포·클러스터 마커를 표시한다. 상권 분석 하위 탭에서는 선택 상권의 별도 지도에 건물 윤곽을 지연 로드하고, 선택 후에도 지도 이동은 제한하지 않는다.
...
- **Outline Variant:** 상권 분석 하위 탭의 별도 지도에는 외곽 상권 경계선과 배경 지도를 두지 않고 경계 내부에 있는 건물 윤곽만 표시한다. manifest와 교차하는 cell만 지연 로드하며, PNU를 우선하고 좌표·같은 필지·거리 fallback으로 연결한 우세 업종을 색칠한다. 한 건물에 여러 업소가 연결되면 업소 수가 가장 많은 업종을 사용하고, 업종 데이터가 없는 도형에는 hover 정보를 제공하지 않는다. 업종 색 범례는 별도 지도 좌측 하단의 반투명 박스로 표시한다.
...
- **Building Outline State:** 상권 분석 하위 탭의 좌측 레일에 선택 상권·업소·연결 점포 수와 단색 전환을 둔다. 업종 분류는 기본으로 켜고, 업종 색 범례는 별도 지도 좌측 하단에 둔다. 건물 데이터가 없으면 빈 상태로, 점포를 연결하지 못한 도형은 `점포 미연결`로 설명한다.
