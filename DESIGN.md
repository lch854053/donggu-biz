...
- **Behavior:** 주요상권 경계는 항상 표시한다. 폴리곤을 클릭하면 행정동 선택을 초기화하고 주요상권 선택기와 왼쪽 지역 상세를 동기화한 뒤 32px 지도 여백, 최대 줌 16으로 경계에 맞춘다. 행정동 또는 주요상권 선택 시 점포·클러스터 마커를 표시하고, 폐업 업소 표시를 켜면 같은 선택 범위의 폐업 이력을 경고 적색 마커로 겹쳐 놓는다. 상권 분석 하위 탭에서는 선택 상권의 별도 지도에 건물 윤곽을 지연 로드하고, 선택 후에도 지도 이동은 제한하지 않는다.
