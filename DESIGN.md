...
밀도는 높지만 역할은 명확하다. 산세리프 한글은 설명과 조작을 담당하고, 고정폭 숫자는 건수·등록번호·거리·좌표처럼 비교해야 하는 데이터를 정렬한다. 상태색은 의미가 있는 배지와 경고에만 등장하며, 지도와 표도 동일한 표면·선·타입 체계를 공유한다.

**Key Characteristics:**
- 무광 청흑색 표면이 층을 이루는 어두운 업무 환경
- 청색 한 가지로 동작, 선택, 진행 상태를 연결하는 체계
...
- **주의 황색:** 휴업과 해석 유의사항처럼 확인이 필요하지만 실패는 아닌 상태를 표시한다.
...
**The Semantic Pair Rule.** 성공·주의·경고는 전경색과 전용 어두운 배경을 쌍으로 사용하며, 색만으로 상태를 전달하지 않는다.
...
사업자 조회는 유연한 입력 스테이션과 280px 기준 안내를 나란히 배치한다. 상권 분석은 250px 필터 레일, 최소 420px 지도, 290px 분석 레일의 세 구역으로 구성하며 지도 높이는 최소 650px이다. 통계는 네 칸 스트립, 결과는 가로 스크롤 가능한 고밀도 표로 표현한다.

1120px 이하에서는 분석 레일이 지도 아래 전체 폭으로 이동해 세 열의 요약 영역이 된다. 780px 이하에서는 모든 업무 구역이 세로 흐름으로 바뀌고, 통계는 2×2가 되며 지도는 56vh 높이를 확보한다. 기준 안내와 출처 태그처럼 부차적인 정보는 좁은 화면에서 숨기고 핵심 입력, 결과, 필터와 분석을 보존한다. 520px 이하에서는 버튼과 필터가 두 열로 늘어나고 결과 표의 과세 유형 열이 생략된다.
...
- **Behavior:** 선택 지점은 청색 원과 반투명 면으로 반경을 표시하고, 하단 안내문이 현재 거리와 필터 기준을 설명한다.
