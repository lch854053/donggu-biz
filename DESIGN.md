...
- **Statistics Tab:** 통계는 사업자 조회·상가·상권 조회와 동급 상단 탭이다. 조건 한 줄, 세 칸 요약 타일, 전폭 선 그래프, 2열 막대 그래프 네 장, 해석 기준 상자를 세로로 쌓는다. 조건은 그래프마다 두지 않고 위쪽 한 줄에만 두어 모든 그래프가 같은 조건으로 다시 그려진다. 휴업을 고르면 폐업일자가 없으므로 연도 선택을 잠근다.
...
- **Chart Reading:** 그래프는 값을 읽는 유일한 통로가 아니다. 카드마다 `표로 보기`를 접어 두어 같은 값을 표로도 읽을 수 있고, 막대와 연도는 마우스 오버와 키보드 포커스에서 같은 말풍선을 띄운다. 말풍선은 값을 굵게 앞세우고 이름을 뒤에 둔다. 적중 영역은 막대 한 줄 전체와 선 그래프의 연도 구간 전체로 잡아 2px 도형을 정확히 겨냥하지 않아도 되게 한다.
