...
- **Style:** 표 아래 12px 곡률 패널에 15px 굵은 사업장명과 260px 최소 폭 두 열 정의 목록을 놓는다. 항목명은 12px 보조 텍스트, 값은 12px 고정폭 우측 정렬이다.
- **Behavior:** 표의 상세 단추를 누르면 같은 자리에서 내용을 교체하고, 새 조회를 시작하면 닫는다. 520px 이하에서는 한 열로 접는다.
...
- **National Pension Variant:** 통계 화면과 국민연금 조회 안내에도 같은 형식을 쓴다. 사업자등록번호가 앞 6자리까지만, 주소가 건물번호 없이 도로명까지만 제공되어 개별 사업자를 특정할 수 없다는 점, 도로명별 집계가 건물 단위 분포가 아니라는 점, 목록 API에 가입자 수가 없어 모든 수치가 사업장 수라는 점, 미가입 사업장이 빠져 지역 전체 사업체 수와 다르다는 점을 명시한다.
