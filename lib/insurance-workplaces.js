...
function searchableValues(row) {
  return [
    ...
    row.employmentInsurance?.address,
    row.employmentInsurance?.employmentIndustryName,
    row.employmentInsurance?.employmentIndustryName11,
    row.employmentInsurance?.employmentIndustryCode,
    row.employmentInsurance?.businessRegistrationNumber,
    row.employmentInsurance?.workplaceManagementNumber
  ].map(normalized).join(" ");
}

function includesInsuranceStatus(row, status) {
  return [row.employmentInsurance?.industrialStatus, row.employmentInsurance?.employmentStatus].includes(status);
}
...
export function matchesInsuranceWorkplaceCriteria(row, criteria = {}) {
  ...
  if (insuranceType && (!row.employmentInsurance || row.employmentInsurance.insuranceType !== insuranceType)) return false;
...
export function sortInsuranceWorkplaces(rows = [], sortKey = "") {
  const values = {
    ...
    "employment-desc": (row) => nullableNumber(row.employmentInsurance?.employmentWorkerCount),
    "industrial-desc": (row) => nullableNumber(row.employmentInsurance?.industrialWorkerCount)
  };
