...
/** 국민연금 업종을 우선하되, 미상이면 고용·산재 업종으로 보강한다. */
export function insuranceIndustrySectionCodes(row) {
  const sections = new Set([row?.nps?.sectionCode].filter(Boolean));
  for (const section of employmentIndustrySections(row?.employmentInsurance)) sections.add(section);
  return sections;
}
