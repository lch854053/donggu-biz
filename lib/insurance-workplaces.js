...
function hasUnknownEmploymentIndustry(row) {
  return insuranceSourceRows(row).some((sourceRow) => !employmentIndustrySectionCode(sourceRow.employmentIndustryCode11 || sourceRow.employmentIndustryCode));
}
...
export function matchesInsuranceWorkplaceCriteria(row, criteria = {}) {
  ...
  if (sectionCode && row.nps?.sectionCode !== sectionCode && !employmentIndustrySections(row.employmentInsurance).has(sectionCode)) return false;
  if (unknownIndustryOnly && !(row.nps && !row.nps.sectionCode) && !hasUnknownEmploymentIndustry(row.employmentInsurance)) return false;
