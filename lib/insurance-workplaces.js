import { employmentIndustrySectionCode, insuranceStatusKind } from "./employment-insurance.js";
...
/**
 * 지정 목록(사회적기업·협동조합 등)과 사업장명을 같은 키로 맞춘다. 법인 표기와
 * 조합 접미사를 벗겨 "(사)아이티케어복지회"와 "아이티케어복지회"가 만나게 한다.
 */
export function designationNameKey(value) {
  return text(value)
    .replace(/[（(]\s*[주유사재]\s*[)）]/g, "")
    .replace(/㈜/g, "")
    .replace(/주식회사|유한회사|사단법인|재단법인|사회적협동조합|협동조합/g, "")
    .replace(/[\s()（）.,·\-_/]/g, "")
...
function currentInsuranceSourceRows(row) {
  const sourceRows = insuranceSourceRows(row);
  const currentRows = sourceRows.filter((sourceRow) => insuranceStatusKind(sourceRow) === "current");
...
/**
 * 통합 사업장 행에 새겨질 지정유형(사회적기업·협동조합 등)을 designation_matches
 * 스냅샷에서 찾는다. 고용·산재의 전체 사업자번호가 있으면 번호로 정확히 맞추고,
 * 국민연금뿐인 사업장은 이름 키로 맞춘다.
 */
export function resolveWorkplaceDesignations(row, index = {}) {
  const byBusinessNumber = index.byBusinessNumber || {};
  const byName = index.byName || {};
  const found = new Set();
  for (const value of businessNumberValues(row)) {
    for (const kind of byBusinessNumber[value] || []) found.add(kind);
  }
  for (const value of [
    row.nps?.name,
    row.employmentInsurance?.name,
    ...insuranceSourceRows(row.employmentInsurance).map((sourceRow) => sourceRow.name)
  ]) {
    const key = designationNameKey(value);
    if (key) for (const kind of byName[key] || []) found.add(kind);
  }
  return [...found];
}

function employmentIndustrySections(row) {
  return new Set(currentInsuranceSourceRows(row)
...
export function insuranceAdminDongs(row) {
  return new Set([
    row?.nps?.adminDong,
    ...currentInsuranceSourceRows(row?.employmentInsurance).map((sourceRow) => sourceRow.adminDong)
...
export function matchesInsuranceWorkplaceCriteria(row, criteria = {}) {
  ...
  if (adminDong && !insuranceAdminDongs(row).has(adminDong)) return false;
  if (source === "nps" && !row.nps) return false;
  if (source === "employment" && !row.employmentInsurance) return false;
  // 선택한 지정유형(사회적기업·협동조합 등) 중 하나라도 걸리면 통과한다.
  if (designations.length && !(row.designations || []).some((kind) => designations.includes(kind))) return false;

  // 종료된 고용·산재 관리기록만 남은 회사는 기본 목록에서 숨기고,
  // 현재 기록이 있거나 과거 기록 포함을 선택한 경우에만 보여 준다.
  if (!includeHistorical && !row.nps && row.employmentInsurance) {
    const sourceRows = insuranceSourceRows(row.employmentInsurance);
    if (sourceRows.length && sourceRows.every((sourceRow) => insuranceStatusKind(sourceRow) === "historical")) return false;
  }
...
export function sortInsuranceWorkplaces(rows = [], sortKey = "") {
  const workerCount = (row, kind) => {
    const values = currentInsuranceSourceRows(row.employmentInsurance)
