import proj4 from "proj4";
import { normalizeAddressLookupKey, normalizeAdminDongName } from "./admin-dong.js";

const EPSG_5174 = "+proj=tmerc +lat_0=38 +lon_0=127.002890277778 +k=1 +x_0=200000 +y_0=500000 +ellps=bessel +towgs84=-145.907,505.034,685.756,-1.162,2.347,1.592,6.342 +units=m +no_defs +type=crs";
const WGS84 = "EPSG:4326";
const EARTH_RADIUS_METERS = 6_371_000;

export const LOCALDATA_ADMIN_CODE = "5805000";
export const LOCALDATA_PAGE_SIZE = 100;

export const LOCALDATA_SOURCES = Object.freeze([
  {
    slug: "general_restaurants",
    datasetId: "15154916",
    title: "행정안전부_식품_일반음식점 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/general_restaurants/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "rest_cafes",
    datasetId: "15154921",
    title: "행정안전부_식품_휴게음식점 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/rest_cafes/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "bakeries",
    datasetId: "15155252",
    title: "행정안전부_식품_제과점영업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/bakeries/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "beauty_salons",
    datasetId: "15154918",
    title: "행정안전부_생활_미용업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/beauty_salons/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "S207",
    middleName: "이용·미용"
  },
  {
    slug: "barber_shops",
    datasetId: "15154922",
    title: "행정안전부_생활_이용업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/barber_shops/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "S207",
    middleName: "이용·미용"
  },
  {
    slug: "tobacco_retailers",
    datasetId: "15155031",
    title: "행정안전부_기타_담배소매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tobacco_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "G204",
    middleName: "종합 소매"
  },
  {
    slug: "large_scale_retail_stores",
    datasetId: "15154948",
    title: "행정안전부_생활_대규모점포 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/large_scale_retail_stores/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "lodgings",
    datasetId: "15155124",
    title: "행정안전부_문화_숙박업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/lodgings/info",
    largeCode: "I1",
    largeName: "숙박",
    middleCode: "I101",
    middleName: "일반 숙박"
  },
  {
    slug: "karaoke_rooms",
    datasetId: "15155135",
    title: "행정안전부_문화_노래연습장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/karaoke_rooms/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "R104",
    middleName: "유원지·오락"
  },
  {
    slug: "pharmacies",
    datasetId: "15154822",
    title: "행정안전부_건강_약국 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/pharmacies/info",
    largeCode: "Q1",
    largeName: "보건의료",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "clinics",
    datasetId: "15154874",
    title: "행정안전부_건강_의원 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/clinics/info",
    largeCode: "Q1",
    largeName: "보건의료",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "hospitals",
    datasetId: "15154458",
    title: "행정안전부_건강_병원 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/hospitals/info",
    largeCode: "Q1",
    largeName: "보건의료",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "optical_shops",
    datasetId: "15154899",
    title: "행정안전부_건강_안경업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/optical_shops/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_hospitals",
    datasetId: "15154952",
    title: "행정안전부_동물_동물병원 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_hospitals/info",
    largeCode: "Q1",
    largeName: "보건의료",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_pharmacies",
    datasetId: "15155272",
    title: "행정안전부_동물_동물약국 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_pharmacies/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "pet_grooming",
    datasetId: "15154944",
    title: "행정안전부_동물_동물미용업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/pet_grooming/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_sales",
    datasetId: "15155083",
    title: "행정안전부_동물_동물판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_sales/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_boarding",
    datasetId: "15155055",
    title: "행정안전부_동물_동물위탁관리업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_boarding/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "fitness_centers",
    datasetId: "15155077",
    title: "행정안전부_생활_체력단련장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/fitness_centers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "martial_arts_dojo",
    datasetId: "15155085",
    title: "행정안전부_생활_체육도장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/martial_arts_dojo/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "golf_practice_ranges",
    datasetId: "15154975",
    title: "행정안전부_생활_골프연습장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/golf_practice_ranges/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "billiard_halls",
    datasetId: "15155011",
    title: "행정안전부_생활_당구장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/billiard_halls/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "swimming_pools",
    datasetId: "15155038",
    title: "행정안전부_생활_수영장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/swimming_pools/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "comprehensive_sports_facilities",
    datasetId: "15155071",
    title: "행정안전부_생활_종합체육시설업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/comprehensive_sports_facilities/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "public_baths",
    datasetId: "15155091",
    title: "행정안전부_생활_목욕장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/public_baths/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "laundries",
    datasetId: "15154927",
    title: "행정안전부_생활_세탁업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/laundries/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "pc_bangs",
    datasetId: "15154951",
    title: "행정안전부_문화_인터넷컴퓨터게임시설제공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/pc_bangs/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "general_game_providers",
    datasetId: "15154955",
    title: "행정안전부_문화_일반게임제공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/general_game_providers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "youth_game_providers",
    datasetId: "15154958",
    title: "행정안전부_문화_청소년게임제공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/youth_game_providers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "mixed_game_providers",
    datasetId: "15154945",
    title: "행정안전부_문화_복합유통게임제공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/mixed_game_providers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "entertainment_bars",
    datasetId: "15154890",
    title: "행정안전부_식품_유흥주점영업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/entertainment_bars/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "singing_bars",
    datasetId: "15154883",
    title: "행정안전부_식품_단란주점영업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/singing_bars/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "instant_food_processors",
    datasetId: "15155245",
    title: "행정안전부_식품_즉석판매제조가공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/instant_food_processors/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "food_vending_machines",
    datasetId: "15155144",
    title: "행정안전부_식품_식품자동판매기업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/food_vending_machines/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "food_repackagers",
    datasetId: "15155126",
    title: "행정안전부_식품_식품소분업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/food_repackagers/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "other_food_retailers",
    datasetId: "15155170",
    title: "행정안전부_식품_식품판매업(기타) 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/other_food_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "health_functional_food_general_retailers",
    datasetId: "15155221",
    title: "행정안전부_식품_건강기능식품일반판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/health_functional_food_general_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "tourist_accommodations",
    datasetId: "15155090",
    title: "행정안전부_문화_관광숙박업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tourist_accommodations/info",
    largeCode: "I1",
    largeName: "숙박",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "rural_homestays",
    datasetId: "15155113",
    title: "행정안전부_문화_농어촌민박업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/rural_homestays/info",
    largeCode: "I1",
    largeName: "숙박",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "tourist_pensions",
    datasetId: "15155103",
    title: "행정안전부_문화_관광펜션업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tourist_pensions/info",
    largeCode: "I1",
    largeName: "숙박",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "over_the_counter_medicine_stores",
    datasetId: "15154791",
    title: "행정안전부_건강_안전상비의약품 판매업소 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/over_the_counter_medicine_stores/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "oil_retailers",
    datasetId: "15155253",
    title: "행정안전부_기타_석유판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/oil_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "petroleum_alt_fuel_retailers",
    datasetId: "15155258",
    title: "행정안전부_기타_석유및석유대체연료판매업체 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/petroleum_alt_fuel_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "dance_halls",
    datasetId: "15155022",
    title: "행정안전부_문화_무도장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/dance_halls/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "dance_academies",
    datasetId: "15155029",
    title: "행정안전부_교육_무도학원업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/dance_academies/info",
    largeCode: "P1",
    largeName: "교육",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "postpartum_care",
    datasetId: "15154981",
    title: "행정안전부_건강_산후조리업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/postpartum_care/info",
    largeCode: "Q1",
    largeName: "보건의료",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "paid_job_centers",
    datasetId: "15155093",
    title: "행정안전부_직업_유료직업소개소 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/paid_job_centers/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "free_job_centers",
    datasetId: "15155099",
    title: "행정안전부_직업_무료직업소개소 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/free_job_centers/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "tourism_businesses",
    datasetId: "15155130",
    title: "행정안전부_문화_관광사업자 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tourism_businesses/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "pay_as_you_throw_bag_retailers",
    datasetId: "15155015",
    title: "행정안전부_기타_쓰레기종량제봉투판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/pay_as_you_throw_bag_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "medical_related_businesses",
    datasetId: "15154864",
    title: "행정안전부_건강_의료유사업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/medical_related_businesses/info",
    largeCode: "Q1",
    largeName: "보건의료",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "tourist_restaurants",
    datasetId: "15154897",
    title: "행정안전부_식품_관광식당 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tourist_restaurants/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "foreigners_entertainment_restaurants",
    datasetId: "15154910",
    title: "행정안전부_식품_외국인전용유흥음식점업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/foreigners_entertainment_restaurants/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "tourist_entertainment_restaurants",
    datasetId: "15154903",
    title: "행정안전부_식품_관광유흥음식점업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tourist_entertainment_restaurants/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "tourist_theater_entertainment",
    datasetId: "15154983",
    title: "행정안전부_문화_관광극장유흥업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tourist_theater_entertainment/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "printing_shops",
    datasetId: "15155014",
    title: "행정안전부_기타_인쇄사 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/printing_shops/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "publishers",
    datasetId: "15155020",
    title: "행정안전부_기타_출판사 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/publishers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "performance_halls",
    datasetId: "15154966",
    title: "행정안전부_문화_공연장 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/performance_halls/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "movie_theaters",
    datasetId: "15154848",
    title: "행정안전부_문화_영화상영관 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/movie_theaters/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "foreigner_city_homestays",
    datasetId: "15155139",
    title: "행정안전부_문화_외국인관광도시민박업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/foreigner_city_homestays/info",
    largeCode: "I1",
    largeName: "숙박",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "medical_device_sales_rental",
    datasetId: "15154923",
    title: "행정안전부_건강_의료기기판매(임대)업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/medical_device_sales_rental/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "dental_labs",
    datasetId: "15154933",
    title: "행정안전부_건강_치과기공소 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/dental_labs/info",
    largeCode: "Q1",
    largeName: "보건의료",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "livestock_retail",
    datasetId: "15154871",
    title: "행정안전부_식품_축산판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/livestock_retail/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  }
,
  {
    slug: "museums_and_art_galleries",
    datasetId: "15155146",
    title: "행정안전부_문화_박물관 및 미술관 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/museums_and_art_galleries/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "registered_sports_facilities",
    datasetId: "15155018",
    title: "행정안전부_생활_등록체육시설업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/registered_sports_facilities/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "pop_culture_art_planners",
    datasetId: "15155155",
    title: "행정안전부_문화_대중문화예술기획업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/pop_culture_art_planners/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "group_meal_facilities",
    datasetId: "15155168",
    title: "행정안전부_식품_집단급식소 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/group_meal_facilities/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "contract_catering",
    datasetId: "15155159",
    title: "행정안전부_식품_위탁급식영업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/contract_catering/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "group_meal_food_retailers",
    datasetId: "15154784",
    title: "행정안전부_식품_집단급식소식품판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/group_meal_food_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "food_manufacturing_processors",
    datasetId: "15155150",
    title: "행정안전부_식품_식품제조가공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/food_manufacturing_processors/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_breeding",
    datasetId: "15154957",
    title: "행정안전부_동물_동물생산업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_breeding/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_cremation",
    datasetId: "15155065",
    title: "행정안전부_동물_동물장묘업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_cremation/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_transport",
    datasetId: "15155024",
    title: "행정안전부_동물_동물운송업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_transport/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "ecommerce_businesses",
    datasetId: "15154963",
    title: "행정안전부_생활_통신판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/ecommerce_businesses/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "outdoor_advertising_companies",
    datasetId: "15155004",
    title: "행정안전부_기타_옥외광고업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/outdoor_advertising_companies/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "comprehensive_travel_agencies",
    datasetId: "15154829",
    title: "행정안전부_문화_종합여행업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/comprehensive_travel_agencies/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "N105",
    middleName: "여행사·보조"
  },
  {
    slug: "domestic_travel_agencies",
    datasetId: "15154821",
    title: "행정안전부_문화_국내여행업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/domestic_travel_agencies/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "N105",
    middleName: "여행사·보조"
  },
  {
    slug: "domestic_international_travel_agencies",
    datasetId: "15154825",
    title: "행정안전부_문화_국내외여행업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/domestic_international_travel_agencies/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "N105",
    middleName: "여행사·보조"
  },
  {
    slug: "health_functional_food_specialty_retailers",
    datasetId: "15155173",
    title: "행정안전부_식품_건강기능식품유통전문판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/health_functional_food_specialty_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "medical_device_repair",
    datasetId: "15154913",
    title: "행정안전부_건강_의료기기수리업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/medical_device_repair/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "tobacco_wholesalers",
    datasetId: "15155028",
    title: "행정안전부_기타_담배도매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tobacco_wholesalers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "meat_packers",
    datasetId: "15155169",
    title: "행정안전부_식품_식육포장처리업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/meat_packers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "G205",
    middleName: "식료품 소매"
  },
  {
    slug: "video_viewing_rooms",
    datasetId: "15155128",
    title: "행정안전부_문화_비디오물감상실업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/video_viewing_rooms/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "R104",
    middleName: "유원지·오락"
  },
  {
    slug: "video_streaming_providers",
    datasetId: "15155059",
    title: "행정안전부_문화_비디오물시청제공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/video_streaming_providers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "R104",
    middleName: "유원지·오락"
  },
  {
    slug: "mixed_video_content_providers",
    datasetId: "15154937",
    title: "행정안전부_문화_복합영상물제공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/mixed_video_content_providers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "R104",
    middleName: "유원지·오락"
  },
  {
    slug: "distribution_specialty_retailers",
    datasetId: "15155251",
    title: "행정안전부_식품_유통전문판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/distribution_specialty_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "food_freezing_refrigeration",
    datasetId: "15155164",
    title: "행정안전부_식품_식품냉동냉장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/food_freezing_refrigeration/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "food_additive_manufacturers",
    datasetId: "15155162",
    title: "행정안전부_식품_식품첨가물제조업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/food_additive_manufacturers/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "edible_ice_retailers",
    datasetId: "15155226",
    title: "행정안전부_식품_식용얼음판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/edible_ice_retailers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "container_packaging_manufacturers",
    datasetId: "15155262",
    title: "행정안전부_식품_용기및포장지제조업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/container_packaging_manufacturers/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "food_transporters",
    datasetId: "15155133",
    title: "행정안전부_식품_식품운반업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/food_transporters/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "livestock_processing",
    datasetId: "15154842",
    title: "행정안전부_식품_축산가공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/livestock_processing/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "livestock_storage",
    datasetId: "15154851",
    title: "행정안전부_식품_축산물보관업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/livestock_storage/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "livestock_transport",
    datasetId: "15154861",
    title: "행정안전부_식품_축산물운반업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/livestock_transport/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "door_to_door_sales",
    datasetId: "15154956",
    title: "행정안전부_생활_방문판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/door_to_door_sales/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "telemarketing_sales",
    datasetId: "15154960",
    title: "행정안전부_생활_전화권유판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/telemarketing_sales/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "multilevel_marketing",
    datasetId: "15154939",
    title: "행정안전부_생활_다단계판매업체 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/multilevel_marketing/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "sponsored_door_to_door_sales",
    datasetId: "15154969",
    title: "행정안전부_생활_후원방문판매업체 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/sponsored_door_to_door_sales/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "funeral_service_providers",
    datasetId: "15155088",
    title: "행정안전부_기타_상조업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/funeral_service_providers/info",
    largeCode: "S2",
    largeName: "수리·개인",
    middleCode: "S210",
    middleName: "장례식장"
  },
  {
    slug: "building_sanitation",
    datasetId: "15154949",
    title: "행정안전부_자원환경_건물위생관리업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/building_sanitation/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "N102",
    middleName: "청소·방제"
  },
  {
    slug: "disinfection_companies",
    datasetId: "15154982",
    title: "행정안전부_자원환경_소독업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/disinfection_companies/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "N102",
    middleName: "청소·방제"
  },
  {
    slug: "water_tank_cleaning",
    datasetId: "15155019",
    title: "행정안전부_자원환경_저수조청소업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/water_tank_cleaning/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "N102",
    middleName: "청소·방제"
  },
  {
    slug: "medical_laundry",
    datasetId: "15154932",
    title: "행정안전부_생활_의료기관세탁물처리업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/medical_laundry/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "caregiver_training",
    datasetId: "15155108",
    title: "행정안전부_기타_요양보호사교육기관 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/caregiver_training/info",
    largeCode: "P1",
    largeName: "교육",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "funeral_director_training",
    datasetId: "15155110",
    title: "행정안전부_기타_장례지도사 교육기관 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/funeral_director_training/info",
    largeCode: "P1",
    largeName: "교육",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "emergency_patient_transport",
    datasetId: "15154834",
    title: "행정안전부_건강_응급환자이송업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/emergency_patient_transport/info",
    largeCode: "Q1",
    largeName: "보건의료",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "elevator_maintenance",
    datasetId: "15155095",
    title: "행정안전부_기타_승강기유지관리업체 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/elevator_maintenance/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "game_producers",
    datasetId: "15154931",
    title: "행정안전부_문화_게임물제작업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/game_producers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "game_distributors",
    datasetId: "15154926",
    title: "행정안전부_문화_게임물배급업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/game_distributors/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "film_producers",
    datasetId: "15154875",
    title: "행정안전부_문화_영화제작업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/film_producers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "film_distributors",
    datasetId: "15154837",
    title: "행정안전부_문화_영화배급업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/film_distributors/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "film_importers",
    datasetId: "15154869",
    title: "행정안전부_문화_영화수입업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/film_importers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "film_screenings",
    datasetId: "15154857",
    title: "행정안전부_문화_영화상영업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/film_screenings/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "music_video_producers",
    datasetId: "15154895",
    title: "행정안전부_문화_음반및음악영상물제작업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/music_video_producers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "music_video_distributors",
    datasetId: "15154886",
    title: "행정안전부_문화_음반및음악영상물배급업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/music_video_distributors/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "record_producers",
    datasetId: "15154909",
    title: "행정안전부_문화_음반물제작업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/record_producers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "record_distributors",
    datasetId: "15154902",
    title: "행정안전부_문화_음반물배급업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/record_distributors/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "online_music_services",
    datasetId: "15154879",
    title: "행정안전부_문화_온라인음악서비스제공업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/online_music_services/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "cultural_art_corporations",
    datasetId: "15155142",
    title: "행정안전부_문화_문화예술법인 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/cultural_art_corporations/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "local_culture_centers",
    datasetId: "15155246",
    title: "행정안전부_문화_지방문화원 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/local_culture_centers/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "tourist_performance_halls",
    datasetId: "15154970",
    title: "행정안전부_문화_관광공연장업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/tourist_performance_halls/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "international_convention_planners",
    datasetId: "15155166",
    title: "행정안전부_문화_국제회의기획업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/international_convention_planners/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "international_convention_facilities",
    datasetId: "15155138",
    title: "행정안전부_문화_국제회의시설업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/international_convention_facilities/info",
    largeCode: "N1",
    largeName: "시설관리·임대",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "city_tour_businesses",
    datasetId: "15155152",
    title: "행정안전부_문화_시내순환관광업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/city_tour_businesses/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_exhibition",
    datasetId: "15155075",
    title: "행정안전부_동물_동물전시업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_exhibition/info",
    largeCode: "R1",
    largeName: "예술·스포츠",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "animal_import",
    datasetId: "15154961",
    title: "행정안전부_동물_동물수입업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/animal_import/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "veterinary_drug_wholesalers",
    datasetId: "15154984",
    title: "행정안전부_동물_동물용의약품도매상 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/veterinary_drug_wholesalers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "veterinary_medical_equipment_sales",
    datasetId: "15154971",
    title: "행정안전부_동물_동물용의료용구판매업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/veterinary_medical_equipment_sales/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "feed_manufacturers",
    datasetId: "15155121",
    title: "행정안전부_동물_사료제조업 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/feed_manufacturers/info",
    largeCode: "G2",
    largeName: "소매",
    middleCode: "",
    middleName: ""
  },
  {
    slug: "excellent_restaurant_info",
    datasetId: "15155052",
    title: "행정안전부_모범음식점정보 조회서비스",
    endpoint: "https://apis.data.go.kr/1741000/excellent_restaurant_info/info",
    largeCode: "I2",
    largeName: "음식",
    middleCode: "",
    middleName: ""
  }
]);

function clean(value) {
  return String(value ?? "").trim();
}

function dateDigits(value) {
  return clean(value).replace(/[^0-9]/g, "");
}

function normalizeMatchText(value) {
  return clean(value)
    .normalize("NFKC")
    .replace(/전남광주통합특별시|광주광역시|광주시|광주/g, "")
    .replace(/주식회사|\(주\)|㈜/g, "")
    .replace(/[^\p{L}\p{N}]/gu, "")
    .toLocaleLowerCase("ko");
}

export function normalizeStoreName(value) {
  return normalizeMatchText(value);
}

export function normalizeStoreAddress(value) {
  return normalizeMatchText(normalizeAddressLookupKey(value));
}

export function namesEquivalent(left, right) {
  const a = normalizeStoreName(left);
  const b = normalizeStoreName(right);
  return Boolean(a && b && (a === b || (a.length >= 4 && (a.includes(b) || b.includes(a)))));
}

export function storeDisplayName(store) {
  return [store?.name, store?.branch]
    .map(clean)
    .filter(Boolean)
    .join(" ");
}

export function epsg5174ToWgs84(x, y) {
  if (!clean(x) || !clean(y)) return null;
  const easting = Number(x);
  const northing = Number(y);
  if (!Number.isFinite(easting) || !Number.isFinite(northing)) return null;
  const [longitude, latitude] = proj4(EPSG_5174, WGS84, [easting, northing]);
  if (!Number.isFinite(longitude) || !Number.isFinite(latitude)) return null;
  if (longitude < 120 || longitude > 140 || latitude < 30 || latitude > 45) return null;
  return { longitude, latitude };
}

export function distanceMeters(left, right) {
  if (!Number.isFinite(left?.longitude) || !Number.isFinite(left?.latitude)
    || !Number.isFinite(right?.longitude) || !Number.isFinite(right?.latitude)) return Infinity;
  const lat1 = left.latitude * Math.PI / 180;
  const lat2 = right.latitude * Math.PI / 180;
  const dLat = lat2 - lat1;
  const dLon = (right.longitude - left.longitude) * Math.PI / 180;
  const a = Math.sin(dLat / 2) ** 2
    + Math.cos(lat1) * Math.cos(lat2) * Math.sin(dLon / 2) ** 2;
  return 2 * EARTH_RADIUS_METERS * Math.asin(Math.sqrt(a));
}

export function parseLocaldataResponse(payload) {
  const response = payload?.response;
  const header = response?.header || {};
  const body = response?.body || {};
  const resultCode = clean(header.resultCode);
  if (resultCode !== "0") throw new Error(`${resultCode || "UNKNOWN"}: ${clean(header.resultMsg) || "인허가 API 오류"}`);
  const rawItems = body.items?.item || [];
  return {
    totalCount: Number(body.totalCount || 0),
    pageNo: Number(body.pageNo || 0),
    numOfRows: Number(body.numOfRows || 0),
    items: Array.isArray(rawItems) ? rawItems : [rawItems]
  };
}

function licenseType(source, item) {
  if (source.slug === "excellent_restaurant_info") return "모범음식점";
  if (source.slug === "beauty_salons") return clean(item.BZSTAT_SE_NM) || clean(item.SNTTN_BZSTAT_NM) || "미용업";
  if (source.slug === "barber_shops") return clean(item.BZSTAT_SE_NM) || "이용업";
  if (source.slug === "tobacco_retailers") return clean(item.CVLCPT_KND_NM) || "담배소매업";
  if (source.slug === "lodgings" || source.slug === "karaoke_rooms") return clean(item.BZSTAT_SE_NM) || source.title.split("_").at(-1).replace(" 조회서비스", "");
  return clean(item.BZSTAT_SE_NM) || source.title.split("_").at(-1).replace(" 조회서비스", "");
}

function licenseSmallName(source, item) {
  const type = licenseType(source, item);
  if (source.slug === "excellent_restaurant_info") return "모범음식점";
  if (source.slug === "beauty_salons" && /피부/.test(`${type} ${clean(item.SNTTN_BZSTAT_NM)}`)) return "피부 관리실";
  if (source.slug === "beauty_salons" && /네일/.test(`${type} ${clean(item.SNTTN_BZSTAT_NM)}`)) return "네일아트";
  if (source.slug === "beauty_salons") return "미용실";
  if (source.slug === "barber_shops") return "이용업";
  if (source.slug === "tobacco_retailers") return "담배소매업";
  if (source.slug === "lodgings") return "숙박업";
  if (source.slug === "karaoke_rooms") return "노래방";
  return type;
}

export const LOCALDATA_STATUS_CODES = Object.freeze({
  active: "01",
  suspended: "02",
  closed: "03"
});

// 원천마다 판매상태 코드 체계가 다르다. 표준(01 영업·02 휴업·03 폐업)과 다른 원천만 적는다.
export const LOCALDATA_STATUS_CODE_EXCEPTIONS = Object.freeze({
  // 모범음식점은 휴업 코드가 없고 02를 폐업으로 쓴다.
  excellent_restaurant_info: Object.freeze({ active: "01", suspended: "", closed: "02" })
});

export function statusCodesFor(source) {
  return (source && LOCALDATA_STATUS_CODE_EXCEPTIONS[source.slug]) || LOCALDATA_STATUS_CODES;
}

export function statusCodeFor(source, kind) {
  return statusCodesFor(source)[kind] || "";
}

export function licenseStatusKind(item, source) {
  const codes = statusCodesFor(source);
  const code = clean(item.SALS_STTS_CD);
  if (code === codes.active) return "active";
  if (codes.suspended && code === codes.suspended) return "suspended";
  if (codes.closed && code === codes.closed) return "closed";
  if (code) return "other";
  const name = clean(item.SALS_STTS_NM);
  if (!name) return "";
  if (/폐업|취소|말소/.test(name)) return "closed";
  if (/휴업/.test(name)) return "suspended";
  if (/영업|정상/.test(name)) return "active";
  return "other";
}

export function isActiveLicense(item, source) {
  return licenseStatusKind(item, source) === "active";
}

export function isClosedLicense(item, source) {
  return licenseStatusKind(item, source) === "closed";
}

export function isSuspendedLicense(item, source) {
  return licenseStatusKind(item, source) === "suspended";
}

// 원천마다 업소명·지번주소·인허가일자 필드 이름이 다르면 여기서 접는다.
const LICENSE_FIELD_ALIASES = Object.freeze({
  excellent_restaurant_info: Object.freeze({ name: "BSNSSP_NM", lotAddress: "LCTN_ADDR", licenseDate: "APLY_YMD" })
});

export function compactLicense(item, source, { adminDong = "" } = {}) {
  const aliases = LICENSE_FIELD_ALIASES[source.slug] || {};
  const managementNo = clean(item.MNG_NO);
  const name = clean(item[aliases.name] || item.BPLC_NM);
  const address = clean(item.ROAD_NM_ADDR) || clean(item[aliases.lotAddress] || item.LOTNO_ADDR);
  const coordinates = epsg5174ToWgs84(item.CRD_INFO_X, item.CRD_INFO_Y);
  const fallbackId = `${normalizeStoreName(name)}:${normalizeStoreAddress(address)}`;
  return {
    id: `license:${source.slug}:${managementNo || fallbackId}`,
    source: "행정안전부 지방행정 인허가 데이터",
    sourceSlug: source.slug,
    sourceDatasetId: source.datasetId,
    sourceName: source.title,
    licenseId: managementNo,
    sourceAdminCode: clean(item.OPN_ATMY_GRP_CD),
    name,
    branch: "",
    largeCode: source.largeCode,
    largeName: source.largeName,
    middleCode: source.middleCode,
    middleName: source.middleName,
    smallCode: "",
    smallName: licenseSmallName(source, item),
    adminDong: adminDong || normalizeAdminDongName(address.replace(/[(),]/g, " ")),
    legalDong: "",
    address,
    lotAddress: clean(item[aliases.lotAddress] || item.LOTNO_ADDR),
    pnu: "",
    legacyPnu: "",
    buildingNo: "",
    buildingName: "",
    floor: "",
    longitude: coordinates?.longitude ?? null,
    latitude: coordinates?.latitude ?? null,
    licenseType: licenseType(source, item),
    statusCode: clean(item.SALS_STTS_CD),
    statusName: clean(item.SALS_STTS_NM),
    detailStatusName: clean(item.DTL_SALS_STTS_NM),
    licenseDate: clean(item[aliases.licenseDate] || item.LCPMT_YMD),
    closedDate: clean(item.CLSBIZ_YMD),
    ...(source.slug === "excellent_restaurant_info" ? {
      designatedAt: clean(item.DSGN_YMD),
      reDesignatedAt: clean(item.RE_DSGN_YMD),
      revokedAt: clean(item.DSGN_RTRCN_YMD),
      revokedReason: clean(item.DSGN_RTRCN_RSN),
      principalFoodKind: clean(item.PRINC_FD_KND)
    } : {}),
    lastModifiedAt: clean(item.LAST_MDFCN_PNT || item.DAT_UPDT_PNT)
  };
}

function exactStoreAddressKeys(store) {
  return [...new Set([store?.address, store?.lotAddress]
    .map(normalizeStoreAddress)
    .filter(Boolean))];
}

function normalizedAddressParts(value) {
  const normalized = normalizeAddressLookupKey(value);
  if (!normalized) return [];
  const parts = [normalized.split(",")[0].trim()];
  const parcel = normalized.match(/(?:^|\s)([^\s,]+(?:동|가))\s+(\d+(?:-\d+)?)(?:\s*번지)?(?:\s+(\d+)\s*호)?/);
  if (parcel) parts.push(`${parcel[1]} ${parcel[2]}${parcel[3] ? `-${parcel[3]}` : ""}`);
  return [...new Set(parts.map(normalizeStoreAddress).filter(Boolean))];
}

function storeAddressKeys(store) {
  return [...new Set([store?.address, store?.lotAddress].flatMap(normalizedAddressParts))];
}

function sameStoreLocation(left, right) {
  const leftKeys = new Set(storeAddressKeys(left));
  return storeAddressKeys(right).some((key) => leftKeys.has(key));
}

function addressNameKeys(store, { exact = false } = {}) {
  const name = normalizeStoreName(storeDisplayName(store));
  const addresses = exact ? exactStoreAddressKeys(store) : storeAddressKeys(store);
  return name ? addresses.map((address) => `${name}|${address}`) : [];
}

function nameCoordinateKey(store) {
  const name = normalizeStoreName(storeDisplayName(store));
  if (!name || !Number.isFinite(store?.longitude) || !Number.isFinite(store?.latitude)) return "";
  return `${name}|${store.longitude}|${store.latitude}`;
}

function nameLookupKeys(name) {
  if (!name) return [];
  const keys = [name];
  if (name.length >= 4) {
    for (let index = 0; index <= name.length - 4; index += 1) {
      keys.push(name.slice(index, index + 4));
    }
  }
  return [...new Set(keys)];
}

export function deduplicateLicenseStores(stores) {
  const unique = [];
  const byKey = new Map();
  const byLocation = new Map();
  const byName = new Map();
  const addIndex = (index, key, value) => {
    if (!key) return;
    if (!index.has(key)) index.set(key, new Set());
    index.get(key).add(value);
  };
  const addStoreIndexes = (store, index) => {
    for (const key of addressNameKeys(store)) addIndex(byKey, key, index);
    for (const key of storeAddressKeys(store)) addIndex(byLocation, key, index);
    for (const key of nameLookupKeys(normalizeStoreName(storeDisplayName(store)))) addIndex(byName, key, index);
  };
  for (const store of stores || []) {
    if (!storeDisplayName(store)) continue;
    const keys = addressNameKeys(store);
    const name = normalizeStoreName(storeDisplayName(store));
    const candidateIndexes = new Set();
    for (const key of keys) {
      for (const index of byKey.get(key) || []) candidateIndexes.add(index);
    }
    for (const key of storeAddressKeys(store)) {
      for (const index of byLocation.get(key) || []) candidateIndexes.add(index);
    }
    for (const key of nameLookupKeys(name)) {
      for (const index of byName.get(key) || []) candidateIndexes.add(index);
    }
    const existingIndex = [...candidateIndexes]
      .sort((left, right) => left - right)
      .find((index) => namesEquivalent(storeDisplayName(unique[index]), storeDisplayName(store))
        && (sameStoreLocation(unique[index], store) || distanceMeters(unique[index], store) <= 30));
    if (existingIndex !== undefined) {
      unique[existingIndex] = mergeLicenseStore(unique[existingIndex], store);
      addStoreIndexes(store, existingIndex);
      continue;
    }
    const index = unique.push(store) - 1;
    addStoreIndexes(store, index);
  }
  return unique;
}

export function deduplicateBaseStores(stores) {
  const unique = [];
  const byKey = new Map();
  const byCoordinate = new Map();
  for (const store of stores || []) {
    if (!storeDisplayName(store)) {
      unique.push(store);
      continue;
    }
    const keys = addressNameKeys(store, { exact: true });
    const addressIndex = keys
      .map((key) => byKey.get(key))
      .find((index) => index !== undefined);
    const coordinateKey = nameCoordinateKey(store);
    const existingIndex = addressIndex ?? (coordinateKey ? byCoordinate.get(coordinateKey) : undefined);
    if (existingIndex !== undefined) {
      unique[existingIndex] = mergeBaseStore(unique[existingIndex], store);
      for (const key of keys) byKey.set(key, existingIndex);
      if (coordinateKey) byCoordinate.set(coordinateKey, existingIndex);
      continue;
    }
    const index = unique.push(store) - 1;
    for (const key of keys) byKey.set(key, index);
    if (coordinateKey) byCoordinate.set(coordinateKey, index);
  }
  return unique;
}

function mergeBaseStore(left, right) {
  const merged = { ...left };
  for (const [key, value] of Object.entries(right || {})) {
    if (!merged[key] && value) merged[key] = value;
  }
  return merged;
}

function mergeLicenseStore(left, right) {
  return {
    ...left,
    sourceSlugs: [...new Set([...(left.sourceSlugs || [left.sourceSlug]), ...(right.sourceSlugs || [right.sourceSlug])])].filter(Boolean),
    licenseIds: [...new Set([...(left.licenseIds || [left.licenseId]), ...(right.licenseIds || [right.licenseId])])].filter(Boolean),
    sourceDatasetIds: [...new Set([...(left.sourceDatasetIds || [left.sourceDatasetId]), ...(right.sourceDatasetIds || [right.sourceDatasetId])])].filter(Boolean)
  };
}

// 매칭된 인허가가 어느 원천에서 왔는지 기존 매장 행에 새긴다. 좌표가 응답에 없어
// 독립 행을 못 만드는 원천(모범음식점 등)도 매칭에 성공하면 출처를 잃지 않는다.
function stampMatchedLicense(store, license) {
  store.sourceSlugs = [...new Set([...(store.sourceSlugs || [store.sourceSlug]), ...(license.sourceSlugs || [license.sourceSlug])])].filter(Boolean);
  store.licenseIds = [...new Set([...(store.licenseIds || []), license.licenseId])].filter(Boolean);
  store.sourceDatasetIds = [...new Set([...(store.sourceDatasetIds || []), license.sourceDatasetId])].filter(Boolean);
}

function buildStoreIndex(stores) {
  const byName = new Map();
  const byAddress = new Map();
  const byLocation = new Map();
  const rows = stores || [];
  const add = (map, key, store) => {
    if (!key) return;
    if (!map.has(key)) map.set(key, []);
    map.get(key).push(store);
  };
  for (const store of rows) {
    const name = normalizeStoreName(storeDisplayName(store));
    if (name) {
      add(byName, name, store);
    }
    for (const address of exactStoreAddressKeys(store)) add(byAddress, address, store);
    for (const address of storeAddressKeys(store)) add(byLocation, address, store);
  }
  return { byName, byAddress, byLocation, stores: rows };
}

function findLicenseMatch(license, index, { includeCategory = true } = {}) {
  const name = normalizeStoreName(storeDisplayName(license));
  const addressKeys = exactStoreAddressKeys(license);
  const addressCandidates = [...new Set(addressKeys.flatMap((key) => index.byAddress.get(key) || []))];
  const locationKeys = storeAddressKeys(license);
  const locationCandidates = [...new Set(locationKeys.flatMap((key) => index.byLocation.get(key) || []))];
  const exactAddress = locationCandidates.find((store) => namesEquivalent(storeDisplayName(license), storeDisplayName(store)));
  if (exactAddress) return { store: exactAddress, type: "name-address", distanceMeters: distanceMeters(license, exactAddress) };

  if (includeCategory) {
    const sameCategory = addressCandidates.filter((store) => store.largeCode === license.largeCode);
    if (sameCategory.length === 1) {
      return { store: sameCategory[0], type: "category-address", distanceMeters: distanceMeters(license, sameCategory[0]) };
    }
  }

  const nameCandidates = index.byName.get(name) || [];
  const exactName = nameCandidates.find((store) => sameStoreLocation(license, store));
  if (exactName) return { store: exactName, type: "name-address", distanceMeters: distanceMeters(license, exactName) };

  const nearbyName = nameCandidates
    .map((store) => ({ store, distanceMeters: distanceMeters(license, store) }))
    .sort((left, right) => left.distanceMeters - right.distanceMeters)[0];
  if (nearbyName && nearbyName.distanceMeters <= 30) return { ...nearbyName, type: "name-coordinate" };

  const nearbyAddress = locationCandidates
    .map((store) => ({ store, distanceMeters: distanceMeters(license, store) }))
    .sort((left, right) => left.distanceMeters - right.distanceMeters)[0];
  if (nearbyAddress && nearbyAddress.distanceMeters <= 20
    && namesEquivalent(storeDisplayName(license), storeDisplayName(nearbyAddress.store))) {
    return { ...nearbyAddress, type: "name-address" };
  }
  return null;
}

export function deduplicateStoreSources(baseStores, licenseStores) {
  const baseRows = baseStores || [];
  const licenseRows = licenseStores || [];
  const uniqueBase = deduplicateBaseStores(baseRows);
  const uniqueLicenses = deduplicateLicenseStores(licenseRows);
  const index = buildStoreIndex(uniqueBase);
  const matched = [];
  const added = [];
  for (const license of uniqueLicenses) {
    const match = findLicenseMatch(license, index, { includeCategory: false });
    if (match) matched.push({ license, ...match });
    else added.push(license);
  }
  for (const { license, store } of matched) stampMatchedLicense(store, license);
  return {
    stores: [...uniqueBase, ...added],
    baseStores: uniqueBase,
    licenseStores: uniqueLicenses,
    added,
    matched,
    baseDuplicatesRemoved: baseRows.length - uniqueBase.length,
    licenseDuplicatesRemoved: licenseRows.length - uniqueLicenses.length,
    matchedCount: matched.length,
    duplicateRowsRemoved: baseRows.length - uniqueBase.length
      + licenseRows.length - uniqueLicenses.length
      + matched.length
  };
}

export function compareStoreSources(baseStores, licenseStores, { baseIndex } = {}) {
  const uniqueLicenses = deduplicateLicenseStores(licenseStores);
  // mergeStoreSources는 자기 출력과 같은 객체를 가리키는 색인을 넘겨 matched.store가
  // 출력 행의 참조가 되게 한다. 내부에서 base를 다시 접으면 참조가 어긋난다.
  const index = baseIndex || buildStoreIndex(deduplicateBaseStores(baseStores));
  const matched = [];
  const unmatched = [];
  for (const license of uniqueLicenses) {
    const match = findLicenseMatch(license, index);
    if (match) matched.push({ license, ...match });
    else unmatched.push(license);
  }
  const bySource = new Map();
  const sourceRow = (sourceSlug) => {
    const row = bySource.get(sourceSlug) || { sourceSlug, raw: 0, unique: 0, matched: 0, added: 0 };
    bySource.set(sourceSlug, row);
    return row;
  };
  for (const license of licenseStores || []) {
    for (const sourceSlug of license.sourceSlugs || [license.sourceSlug]) sourceRow(sourceSlug).raw += 1;
  }
  for (const license of uniqueLicenses) {
    for (const sourceSlug of license.sourceSlugs || [license.sourceSlug]) sourceRow(sourceSlug).unique += 1;
  }
  for (const { license } of matched) {
    for (const sourceSlug of license.sourceSlugs || [license.sourceSlug]) sourceRow(sourceSlug).matched += 1;
  }
  for (const license of unmatched) {
    for (const sourceSlug of license.sourceSlugs || [license.sourceSlug]) sourceRow(sourceSlug).added += 1;
  }
  return {
    rawLicenseCount: (licenseStores || []).length,
    uniqueLicenseCount: uniqueLicenses.length,
    matchedCount: matched.length,
    addedCount: unmatched.length,
    addedWithCoordinatesCount: unmatched.filter((store) => Number.isFinite(store.longitude) && Number.isFinite(store.latitude)).length,
    addedWithoutCoordinatesCount: unmatched.filter((store) => !Number.isFinite(store.longitude) || !Number.isFinite(store.latitude)).length,
    matchTypeCounts: matched.reduce((counts, row) => ({ ...counts, [row.type]: (counts[row.type] || 0) + 1 }), {}),
    bySource: [...bySource.values()],
    matched,
    unmatched
  };
}

export function mergeStoreSources(baseStores, licenseStores) {
  const uniqueBase = deduplicateBaseStores(baseStores);
  const comparison = compareStoreSources(uniqueBase, licenseStores, { baseIndex: buildStoreIndex(uniqueBase) });
  for (const { license, store } of comparison.matched) stampMatchedLicense(store, license);
  const added = comparison.unmatched.filter((store) => Number.isFinite(store.longitude) && Number.isFinite(store.latitude));
  return {
    stores: [...uniqueBase, ...added],
    added,
    comparison
  };
}

export function latestSourceTimestamp(items) {
  return (items || [])
    .map((item) => clean(item.lastModifiedAt))
    .filter(Boolean)
    .sort((left, right) => dateDigits(right).localeCompare(dateDigits(left)))[0] || "";
}
