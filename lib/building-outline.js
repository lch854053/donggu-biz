...
// 건물 스냅샷에는 같은 PNU가 수백 미터 밖 다른 필지 건물에도 중복으로 붙어 있다(계림1·2동 사례).
// 업소 좌표를 알 때는 PNU가 같아도 이보다 먼 도형은 다른 필지로 보고 연결하지 않는다.
const PNU_MATCH_MAX_METERS = 200;
...
// 좌표가 있는 행들만의 [서, 남, 동, 북] 경계. 하나도 없으면 null.
export function rowsBounds(rows) {
  const bounds = [Infinity, Infinity, -Infinity, -Infinity];
  for (const row of rows || []) {
    if (!Number.isFinite(row?.longitude) || !Number.isFinite(row?.latitude)) continue;
    bounds[0] = Math.min(bounds[0], row.longitude);
    bounds[1] = Math.min(bounds[1], row.latitude);
    bounds[2] = Math.max(bounds[2], row.longitude);
    bounds[3] = Math.max(bounds[3], row.latitude);
  }
  return Number.isFinite(bounds[0]) ? bounds : null;
}

// 경계 숫자 [서, 남, 동, 북]를 닫힌 GeoJSON 폴리곤으로 만든다.
...
export function matchBuildingIndustries(features, stores) {
  ...
  for (const store of stores || []) {
    ...
    const hasPosition = Number.isFinite(store?.longitude) && Number.isFinite(store?.latitude);
    for (const key of [...pnuKeys(store?.pnu), ...pnuKeys(store?.legacyPnu)]) {
      for (const feature of featuresByPnu.get(key) || []) {
        if (hasPosition && geometryDistanceMeters(store.longitude, store.latitude, feature.geometry) > PNU_MATCH_MAX_METERS) continue;
        matches.set(String(feature.id), feature);
      }
    }
    if (!matches.size && hasPosition) {
      for (const feature of features || []) {
