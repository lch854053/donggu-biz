...
export function geometryContainedBy(inner, outer) {
  if (!inner || !outer || !boundsIntersect(geometryBounds(inner), geometryBounds(outer))) return false;
  const rings = geometryRings(inner);
  return rings.length > 0
    && rings.every((ring) => ring.every(([longitude, latitude]) => pointInGeometry(longitude, latitude, outer)));
}
