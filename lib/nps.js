...
/**
 * 목록 검색 결과가 스냅샷의 사업장과 같은 행인지 본다. 검색 결과에는 이름과 번호
 * 앞자리가 우연히 같은 다른 사업장이 섞일 수 있으니 상세조회보다 엄격하게 이름까지
 * 같아야 한다. 이름이 바뀐 사업장은 여기서 걸러지고, 그때는 스냅샷의 seq를 쓴다.
 */
export function isSameWorkplaceListItem(candidate, workplace) {
  return isSameWorkplaceDetail(candidate, workplace)
    && normalizeKeyPart(candidate?.name) === normalizeKeyPart(workplace?.name);
}
