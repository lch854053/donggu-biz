...
/** 사업장 상세 항목. 가입자 수와 고지금액은 상세조회에만 존재한다. */
export function compactWorkplaceDetail(item) {
  return {
    ...
    newSubscriberCount: count(item?.nwAcqzrCnt),
    lostSubscriberCount: count(item?.lssJnngpCnt)
  };
}
...
export function parseNpsXml(xml) {
  ...
  if (authMessage) {
    ...
    throw new Error(`국민연금 API 오류${reason ? ` ${reason}` : ""}: ${authMessage}`);
  }
  const resultCode = tag("resultCode");
  if (resultCode && resultCode !== "00" && resultCode !== "0") {
    throw new Error(`국민연금 API 오류 ${resultCode}: ${tag("resultMsg") || "사유 미상"}`);
  }
...
export function parseNpsResponse(payload) {
  ...
  if (resultCode && resultCode !== "00" && resultCode !== "0") {
    throw new Error(`국민연금 API 오류 ${resultCode}: ${text(header?.resultMsg) || "사유 미상"}`);
  }
