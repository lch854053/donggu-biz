// 국민연금공단 국민연금 가입 사업장 내역 (NpsBplcInfoInqireServiceV2) 응답 정규화와 통계 집계.
// 이 API는 사업자등록번호를 앞 6자리까지만 제공하고 주소는 건물번호 없이 도로명까지만 제공한다.

export const NPS_STATUS_LABELS = {
  1: "등록",
  2: "탈퇴"
};

export const NPS_STYLE_LABELS = {
  1: "법인",
  2: "개인"
};

// 국민연금 사업장 업종코드는 여섯 자리이고 앞 두 자리가 중분류다. 다만 번호 체계가
// 최신 한국표준산업분류(10·11차)가 아니라 2007년 개정 전의 옛 분류를 그대로 쓴다.
// 실제 응답을 보면 452125가 "일반 전기 공사업", 513421이 "서적, 잡지 및 기타 인쇄물
// 도매업"이라 45는 건설업, 51은 도매업이다. 최신 표로 읽으면 건설업이 도매 및 소매업으로,
// 도매업이 운수업으로 한 칸씩 밀리므로 아래 표는 옛 분류 기준으로 둔다.
export const INDUSTRY_SECTIONS = [
  { code: "A", name: "농업 및 임업", from: 1, to: 2 },
  { code: "B", name: "어업", from: 5, to: 5 },
  { code: "C", name: "광업", from: 10, to: 14 },
  { code: "D", name: "제조업", from: 15, to: 37 },
  { code: "E", name: "전기, 가스 및 수도사업", from: 40, to: 41 },
  { code: "F", name: "건설업", from: 45, to: 45 },
  { code: "G", name: "도매 및 소매업", from: 50, to: 52 },
  { code: "H", name: "숙박 및 음식점업", from: 55, to: 55 },
  { code: "I", name: "운수업", from: 60, to: 63 },
  { code: "J", name: "통신업", from: 64, to: 64 },
  { code: "K", name: "금융 및 보험업", from: 65, to: 67 },
  { code: "L", name: "부동산 및 임대업", from: 70, to: 71 },
  { code: "M", name: "사업서비스업", from: 72, to: 74 },
  { code: "N", name: "공공행정, 국방 및 사회보장 행정", from: 75, to: 75 },
  { code: "O", name: "교육 서비스업", from: 80, to: 80 },
  { code: "P", name: "보건 및 사회복지사업", from: 85, to: 85 },
  { code: "Q", name: "기타 공공, 수리 및 개인 서비스업", from: 90, to: 93 },
  { code: "R", name: "가사 서비스업", from: 95, to: 95 },
  { code: "S", name: "국제 및 외국기관", from: 99, to: 99 }
];

// 업종을 담고 있지 않은 자리표시 코드. 어린이집처럼 사업자등록번호가 잡히지 않는 사업장은
// 업종코드 자리에 999999("BIZ_NO미존재사업장")가 오는데, 앞 두 자리만 보면 중분류 99가 되어
// 국제 및 외국기관으로 읽힌다. 업종을 모른다는 뜻이므로 분류하지 않고 미상으로 둔다.
const PLACEHOLDER_INDUSTRY_CODES = new Set(["000000", "999999"]);
const PLACEHOLDER_INDUSTRY_NAMES = new Set(["해당없음", "BIZ_NO미존재사업장"]);

const UNKNOWN_SECTION = { code: "", name: "업종 미상" };

/** 입력값에서 숫자만 남긴 뒤 사업자등록번호 앞 6자리를 돌려준다. API가 6자리까지만 지원한다. */
export function toBizNoPrefix(raw) {
  const digits = String(raw ?? "").replace(/[^0-9]/g, "");
  return digits.slice(0, 6);
}

/** 업종을 담고 있지 않은 자리표시 업종코드인지 본다. */
export function isPlaceholderIndustry(industryCode) {
  return PLACEHOLDER_INDUSTRY_CODES.has(String(industryCode ?? "").replace(/[^0-9]/g, ""));
}

/** 업종코드에서 대분류를 판정한다. 자리표시 코드와 표에 없는 코드는 미상으로 둔다. */
export function industrySection(industryCode) {
  const digits = String(industryCode ?? "").replace(/[^0-9]/g, "");
  if (isPlaceholderIndustry(digits)) return UNKNOWN_SECTION;
  const division = Number(digits.slice(0, 2));
  if (!Number.isInteger(division) || division <= 0) return UNKNOWN_SECTION;
  return INDUSTRY_SECTIONS.find((section) => division >= section.from && division <= section.to) || UNKNOWN_SECTION;
}

// 이 서비스는 광주 동구만 다루므로 주소마다 붙는 시 이름은 읽는 데 도움이 되지 않는다.
const ADDRESS_CITY_PREFIXES = ["광주광역시", "광주시", "광주"];

/** 화면에 쓸 소재지. 지역이 하나로 고정된 서비스라 앞의 시 이름은 덜어낸다. */
export function displayAddress(address) {
  const value = String(address ?? "").trim();
  for (const prefix of ADDRESS_CITY_PREFIXES) {
    if (value.startsWith(`${prefix} `)) return value.slice(prefix.length).trim();
  }
  return value;
}

/** yyyymmdd에서 연도만 숫자로 꺼낸다. 형식이 아니면 null. */
export function ymdYear(value) {
  const digits = String(value ?? "").replace(/[^0-9]/g, "");
  if (digits.length < 4) return null;
  const year = Number(digits.slice(0, 4));
  return Number.isInteger(year) && year > 1000 ? year : null;
}

/**
 * 업종·등록일·가입자 수는 목록 조회에 없어 상세조회로 채운다. 아직 채우지 못한 사업장은
 * 업종을 "미상"으로 단정하면 안 되므로, 상세를 받았는지 여부를 따로 본다.
 */
export function hasIndustryDetail(workplace) {
  return Boolean(workplace?.detailLoaded || workplace?.industryCode);
}

/** 건물번호가 없는 일반화 주소에서 집계 단위로 쓸 "시군구 도로명"을 뽑는다. */
export function addressArea(address) {
  const parts = String(address ?? "").trim().split(/\s+/).filter(Boolean);
  if (!parts.length) return "주소 미상";
  const road = parts[parts.length - 1];
  const district = parts.slice(0, -1).reverse().find((part) => /[시군구]$/.test(part));
  return district ? `${district} ${road}` : road;
}

function text(value) {
  return String(value ?? "").trim();
}

function count(value) {
  const parsed = Number(String(value ?? "").replace(/[^0-9-]/g, ""));
  return Number.isFinite(parsed) ? parsed : 0;
}

/** 업종명 자리에 오는 안내 문구는 업종이 아니므로 비운다. */
function industryName(value) {
  const name = text(value);
  return PLACEHOLDER_INDUSTRY_NAMES.has(name) ? "" : name;
}

function labelFor(map, code, fallback) {
  return map[Number(code)] || (code ? `${fallback}(${code})` : fallback);
}

/** 사업장 목록 항목을 화면과 통계가 함께 쓰는 형태로 정규화한다. */
export function compactWorkplace(item) {
  const industryCode = text(item?.wkplIntpCd);
  const section = industrySection(industryCode);
  const address = text(item?.wkplRoadNmDtlAddr);
  return {
    seq: text(item?.seq),
    dataCreatedMonth: text(item?.dataCrtYm),
    name: text(item?.wkplNm),
    bizNoPrefix: toBizNoPrefix(item?.bzowrRgstNo),
    address,
    area: addressArea(address),
    statusCode: text(item?.wkplJnngStcd),
    statusName: labelFor(NPS_STATUS_LABELS, item?.wkplJnngStcd, "상태 미상"),
    styleCode: text(item?.wkplStylDvcd),
    styleName: labelFor(NPS_STYLE_LABELS, item?.wkplStylDvcd, "형태 미상"),
    industryCode,
    sectionCode: section.code,
    sectionName: section.name,
    sidoCode: text(item?.ldongAddrMgplDgCd),
    sgguCode: text(item?.ldongAddrMgplSgguCd),
    emdCode: text(item?.ldongAddrMgplSgguEmdCd)
  };
}

/**
 * 상세조회 응답이 스냅샷이 가리키는 사업장과 같은지 사업자번호 앞자리로 확인한다.
 * seq는 자료생성월 배치의 일련번호라 시간이 지난 뒤에는 다른 사업장으로 응답할 수
 * 있으므로, 상세 응답을 표시하거나 스냅샷에 섞기 전에 반드시 본다.
 */
export function isSameWorkplaceDetail(detail, workplace) {
  const prefix = text(detail?.bizNoPrefix);
  return prefix !== "" && prefix === text(workplace?.bizNoPrefix);
}

/**
 * 목록 검색 결과에서 스냅샷의 사업장 행을 골라 월별 이력({seq, month})으로 정리한다.
 * 최근 기준월부터 정렬한다. 이름이나 주소가 같은 행은 개명·이전을 지나온 우리
 * 사업장이고, 둘 다 다른 행은 번호 앞자리만 우연히 같은 다른 사업장이므로 버린다.
 * 그런 행뿐이면 이름과 주소가 모두 바뀐 우리 사업장일 테니 남긴다.
 */
export function pickSnapshotWorkplaceRows(items = [], workplace) {
  const rows = items.filter((item) => item?.seq && item?.dataCreatedMonth && isSameWorkplaceDetail(item, workplace));
  const ours = rows.filter((item) =>
    normalizeKeyPart(item.name) === normalizeKeyPart(workplace?.name)
    || normalizeKeyPart(item.address) === normalizeKeyPart(workplace?.address));
  return (ours.length ? ours : rows)
    .sort((left, right) => String(right.dataCreatedMonth).localeCompare(String(left.dataCreatedMonth)))
    .map((item) => ({ seq: item.seq, month: item.dataCreatedMonth }));
}

// 탈퇴하지 않은 사업장의 탈퇴일 자리에 오는 자리표시값이다.
const PLACEHOLDER_DATE = "00010101";

function withdrawalDate(value) {
  const date = text(value);
  return date === PLACEHOLDER_DATE ? "" : date;
}

/**
 * 스냅샷 항목을 화면이 조회 결과처럼 다룰 수 있는 형태로 편다. 상태·형태 이름과 업종
 * 대분류는 스냅샷에 적힌 값을 쓰지 않고 코드에서 다시 판정한다. 분류표를 고쳐도 지난달에
 * 만들어 둔 스냅샷은 옛 판정을 담고 있기 때문이다.
 */
export function hydrateSnapshotWorkplace(item) {
  const industryCode = text(item?.industryCode);
  const section = industrySection(industryCode);
  const address = text(item?.address);
  return {
    seq: text(item?.seq),
    dataCreatedMonth: text(item?.dataCreatedMonth),
    name: text(item?.name),
    bizNoPrefix: text(item?.bizNoPrefix),
    address,
    adminDong: text(item?.adminDong),
    area: addressArea(address),
    statusCode: text(item?.statusCode),
    statusName: labelFor(NPS_STATUS_LABELS, item?.statusCode, "상태 미상"),
    styleCode: text(item?.styleCode),
    styleName: labelFor(NPS_STYLE_LABELS, item?.styleCode, "형태 미상"),
    industryCode,
    industryName: isPlaceholderIndustry(industryCode) ? "" : text(item?.industryName),
    sectionCode: section.code,
    sectionName: section.name,
    registeredDate: text(item?.registeredDate),
    withdrawnDate: withdrawalDate(item?.withdrawnDate),
    subscriberCount: typeof item?.subscriberCount === "number" ? item.subscriberCount : null,
    historyCount: Number(item?.historyCount) || 1,
    historyRows: (item?.historyRows ?? []).filter((row) => row?.seq),
    // 스냅샷은 상세조회까지 마친 자료다. "업종을 모른다"와 "아직 못 받았다"를 가르는 표시다.
    detailLoaded: true
  };
}

/** 값이 아예 없는 항목과 0을 구분한다. 오퍼레이션마다 제공하는 항목이 다르다. */
function optionalCount(value) {
  return text(value) === "" ? null : count(value);
}

/**
 * 사업장 상세 항목. 업종코드·업종명·등록일·가입자 수·고지금액은 상세조회에만 있고,
 * 월별 취득·상실자 수는 기간별 현황 오퍼레이션에서 합쳐 온다.
 */
export function compactWorkplaceDetail(item) {
  return {
    ...compactWorkplace(item),
    industryName: industryName(item?.vldtVlKrnNm),
    registeredDate: text(item?.adptDt),
    withdrawnDate: withdrawalDate(item?.scsnDt),
    subscriberCount: count(item?.jnngpCnt),
    monthlyNoticeAmount: count(item?.crrmmNtcAmt ?? item?.ntcAmt),
    newSubscriberCount: optionalCount(item?.nwAcqzrCnt),
    lostSubscriberCount: optionalCount(item?.lssJnngpCnt)
  };
}

/** 표기 차이를 지운 비교용 문자열. 공백과 괄호·가운뎃점 같은 기호를 털어낸다. */
function normalizeKeyPart(value) {
  return String(value ?? "").replace(/[\s()（）.,·\-]/g, "").toLowerCase();
}

/**
 * 같은 사업장으로 볼 기준. 사업장명·사업자등록번호 앞 6자리·도로명주소가 모두 같으면
 * 같은 사업장으로 본다. 주소를 넣어야 이름과 앞 6자리가 같은 지점들이 뭉개지지 않는다.
 */
export function workplaceIdentity(workplace) {
  return [
    normalizeKeyPart(workplace?.name),
    String(workplace?.bizNoPrefix ?? ""),
    normalizeKeyPart(workplace?.address)
  ].join("|");
}

/**
 * 조건검색. 사업장명·가입상태·사업장 형태는 목록 조회만으로 판정할 수 있지만
 * 업종·등록일·가입자 수는 상세조회를 받아야 알 수 있다. 아직 상세를 못 받은 사업장은
 * 그 조건을 걸었을 때만 빼고, 조건을 걸지 않았으면 그대로 남긴다.
 *
 * 등록일·가입자 수 범위는 값이 null이면 조건을 걸지 않은 것으로 본다. 화면은 슬라이더가
 * 양 끝에 있을 때 null을 넘겨, 아직 상세를 못 받은 사업장이 사라지지 않게 한다.
 */
export function matchesWorkplaceCriteria(workplace, criteria = {}) {
  const {
    name = "",
    includeWithdrawn = false,
    styleCode = "",
    sectionCode = "",
    unknownIndustryOnly = false,
    registeredFromYear = null,
    registeredToYear = null,
    subscriberMin = null,
    subscriberMax = null
  } = criteria;

  if (!includeWithdrawn && text(workplace?.statusCode) !== "1") return false;
  if (styleCode && text(workplace?.styleCode) !== styleCode) return false;
  if (name && !normalizeKeyPart(workplace?.name).includes(normalizeKeyPart(name))) return false;

  const industryKnown = hasIndustryDetail(workplace);
  if (unknownIndustryOnly) {
    if (!industryKnown || industrySection(workplace?.industryCode).code) return false;
  } else if (sectionCode) {
    if (!industryKnown || industrySection(workplace?.industryCode).code !== sectionCode) return false;
  }

  if (registeredFromYear != null || registeredToYear != null) {
    const year = ymdYear(workplace?.registeredDate);
    if (year == null) return false;
    if (registeredFromYear != null && year < registeredFromYear) return false;
    if (registeredToYear != null && year > registeredToYear) return false;
  }

  if (subscriberMin != null || subscriberMax != null) {
    const people = workplace?.subscriberCount;
    if (typeof people !== "number") return false;
    if (subscriberMin != null && people < subscriberMin) return false;
    if (subscriberMax != null && people > subscriberMax) return false;
  }
  return true;
}

const WORKPLACE_SORTS = {
  "registered-asc": { field: "registeredDate", direction: 1 },
  "registered-desc": { field: "registeredDate", direction: -1 },
  "subscriber-asc": { field: "subscriberCount", direction: 1 },
  "subscriber-desc": { field: "subscriberCount", direction: -1 }
};

/** 조회 결과의 표시용 정렬. 값이 없는 사업장은 항상 목록 뒤에 둔다. */
export function sortWorkplaces(workplaces, sortKey = "") {
  const sort = WORKPLACE_SORTS[sortKey];
  if (!sort) return [...(workplaces ?? [])];

  return (workplaces ?? [])
    .map((workplace, index) => ({ workplace, index }))
    .sort((a, b) => {
      const left = a.workplace?.[sort.field];
      const right = b.workplace?.[sort.field];
      const leftMissing = left == null || left === "";
      const rightMissing = right == null || right === "";
      if (leftMissing || rightMissing) {
        if (leftMissing && rightMissing) return a.index - b.index;
        return leftMissing ? 1 : -1;
      }
      const comparison = typeof left === "number" && typeof right === "number"
        ? left - right
        : String(left).localeCompare(String(right), "ko", { numeric: true, sensitivity: "base" });
      return comparison * sort.direction || a.index - b.index;
    })
    .map(({ workplace }) => workplace);
}

/**
 * 이 API는 자료생성년월마다 같은 사업장을 한 건씩 쌓아 돌려준다. 같은 사업장은 기준월이
 * 가장 최근인 한 건으로 접고, 몇 개월치가 있었는지와 기준월 목록을 함께 남긴다.
 * 목록에 없는 사업장을 만들어내지 않도록 순서는 처음 나온 차례를 지킨다.
 */
export function mergeWorkplaceHistory(workplaces) {
  // 이미 한 번 접힌 목록을 다시 접어도 이력이 사라지지 않도록 기준월 목록을 함께 모은다.
  const monthsOf = (workplace) => [workplace?.dataCreatedMonth, ...(workplace?.historyMonths ?? [])].filter(Boolean);
  // 월별 이력을 그래프로 보려면 각 달의 seq가 필요하다. 접으면서 함께 모아 둔다.
  const rowsOf = (workplace) => (workplace?.historyRows?.length
    ? workplace.historyRows
    : [{ seq: workplace?.seq ?? "", month: workplace?.dataCreatedMonth ?? "" }]).filter((row) => row.seq);
  const groups = new Map();
  for (const workplace of workplaces ?? []) {
    const key = workplaceIdentity(workplace);
    const group = groups.get(key);
    if (!group) {
      groups.set(key, { latest: workplace, months: new Set(monthsOf(workplace)), rows: new Map(rowsOf(workplace).map((row) => [row.seq, row])) });
      continue;
    }
    for (const month of monthsOf(workplace)) group.months.add(month);
    for (const row of rowsOf(workplace)) group.rows.set(row.seq, row);
    if (text(workplace.dataCreatedMonth) > text(group.latest.dataCreatedMonth)) group.latest = workplace;
  }
  return [...groups.values()].map(({ latest, months, rows }) => ({
    ...latest,
    historyCount: months.size || 1,
    historyMonths: [...months].sort().reverse(),
    historyRows: [...rows.values()].sort((a, b) => text(b.month).localeCompare(text(a.month)))
  }));
}

// 공공데이터포털 게이트웨이가 돌려주는 오류 코드별 안내. 화면에 그대로 보여줄 수 있는 문장이다.
export const NPS_ERROR_HINTS = {
  "4": "국민연금 API 서버가 제때 응답하지 않았습니다. 잠시 후 다시 시도해 주세요.",
  "12": "폐기되었거나 주소가 바뀐 서비스입니다. 오퍼레이션 주소를 확인하세요.",
  "20": "서비스 접근이 거부되었습니다. 활용신청 승인 상태를 확인하세요.",
  "22": "일일 트래픽 한도를 초과했습니다.",
  "30": "등록되지 않은 서비스키입니다. 인코딩 키가 아닌 디코딩 키를 NPS_SERVICE_KEY에 넣었는지 확인하세요.",
  "31": "활용기간이 만료된 서비스키입니다.",
  "32": "등록되지 않은 IP에서의 호출입니다.",
  "97": "요청 파라미터가 규격과 맞지 않습니다. 파라미터 이름과 값(numOfRows 최대 100, 시군구는 시도를 포함한 5자리 법정동코드)을 확인하세요.",
  "99": "국민연금 API 내부 오류입니다."
};

// 요청을 고쳐 다시 부를 여지가 없는 코드. 키·권한·한도 문제는 재시도해도 같은 답이 온다.
const UNRECOVERABLE_ERROR_CODES = new Set(["12", "20", "22", "30", "31", "32"]);

function npsError(code, message) {
  const error = new Error(message);
  if (code) error.code = String(code);
  return error;
}

/** 오류 코드에 해당하는 안내를 덧붙인 메시지를 만든다. */
export function describeNpsError(error) {
  const hint = NPS_ERROR_HINTS[String(error?.code ?? "")];
  return hint ? `${error.message} (${hint})` : String(error?.message ?? "국민연금 API 오류");
}

/** 파라미터 표기를 바꿔 다시 호출해 볼 만한 오류인지 판정한다. */
export function isRequestShapeError(error) {
  const code = String(error?.code ?? "");
  return Boolean(code) && !UNRECOVERABLE_ERROR_CODES.has(code);
}

const XML_ENTITIES = { amp: "&", lt: "<", gt: ">", quot: '"', apos: "'", "#39": "'" };

function decodeXmlText(value) {
  return String(value ?? "")
    .replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, "$1")
    .replace(/&(amp|lt|gt|quot|apos|#39);/g, (whole, entity) => XML_ENTITIES[entity] ?? whole)
    .trim();
}

/**
 * 이 서비스는 resultType 파라미터와 무관하게 XML로 응답한다. 봉투가 평평해서
 * item 블록과 그 자식 태그만 훑어도 필요한 값을 모두 얻을 수 있다.
 */
export function parseNpsXml(xml) {
  const source = String(xml ?? "");
  const tag = (name) => {
    const match = source.match(new RegExp(`<${name}>([\\s\\S]*?)</${name}>`));
    return match ? decodeXmlText(match[1]) : "";
  };

  const authMessage = tag("returnAuthMsg") || tag("errMsg");
  if (authMessage) {
    const reason = tag("returnReasonCode") || tag("errorCode");
    throw npsError(reason, `국민연금 API 오류${reason ? ` ${reason}` : ""}: ${authMessage}`);
  }
  const resultCode = tag("resultCode");
  if (resultCode && resultCode !== "00" && resultCode !== "0") {
    throw npsError(resultCode, `국민연금 API 오류 ${resultCode}: ${tag("resultMsg") || "사유 미상"}`);
  }

  const items = [...source.matchAll(/<item>([\s\S]*?)<\/item>/g)].map(([, block]) => {
    const record = {};
    for (const [, name, value] of block.matchAll(/<([A-Za-z0-9_]+)>([\s\S]*?)<\/\1>/g)) {
      record[name] = decodeXmlText(value);
    }
    return record;
  });

  // 빈 결과도 헤더나 items 껍데기는 남는다. 둘 다 없으면 이 서비스의 응답이 아니다.
  if (!items.length && !resultCode && !/<items\b/.test(source)) {
    throw new Error("국민연금 API 응답 형식을 해석할 수 없습니다.");
  }
  return {
    items,
    totalCount: count(tag("totalCount")),
    pageNo: count(tag("pageNo")) || 1,
    numOfRows: count(tag("numOfRows")) || items.length
  };
}

/** 본문이 JSON이든 XML이든 같은 형태로 푼다. 어느 쪽도 아니면 오류로 올린다. */
export function parseNpsBody(body) {
  const trimmed = String(body ?? "").trim();
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    let payload;
    try {
      payload = JSON.parse(trimmed);
    } catch {
      throw new Error("국민연금 API 응답 형식을 해석할 수 없습니다.");
    }
    return parseNpsResponse(payload);
  }
  if (trimmed.startsWith("<")) return parseNpsXml(trimmed);
  throw new Error("국민연금 API 응답 형식을 해석할 수 없습니다.");
}

/**
 * data.go.kr 표준 JSON 응답 봉투를 푼다. items가 배열, 단일 객체, 빈 문자열로 오는 경우를 모두 받는다.
 * 정상 응답이 아니면 헤더의 결과 메시지를 담아 throw 한다.
 */
export function parseNpsResponse(payload) {
  const body = payload?.response?.body;
  const header = payload?.response?.header;
  const resultCode = text(header?.resultCode);
  if (resultCode && resultCode !== "00" && resultCode !== "0") {
    throw npsError(resultCode, `국민연금 API 오류 ${resultCode}: ${text(header?.resultMsg) || "사유 미상"}`);
  }
  if (!body) throw new Error("국민연금 API 응답 형식을 해석할 수 없습니다.");
  const raw = body.items?.item ?? body.items ?? [];
  const items = Array.isArray(raw) ? raw : raw && typeof raw === "object" ? [raw] : [];
  return {
    items,
    totalCount: count(body.totalCount),
    pageNo: count(body.pageNo) || 1,
    numOfRows: count(body.numOfRows) || items.length
  };
}
