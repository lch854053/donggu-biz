...
/**
 * 상세조회 응답이 스냅샷이 가리키는 사업장과 같은지 확인한다. 사업자등록번호 앞
 * 6자리는 관할·업종이 같은 사업장끼리 널리 겹치므로(동구만 해도 사업장의 15%가
 * 다른 사업장과 앞자리를 공유한다) 이름까지 같아야 한다. 주소는 건물번호 없는
 * 도로명까지라 같은 길의 다른 사업장과도 겹쳐 판정에 쓰지 않는다. 개명한 사업장은
 * 여기서 걸러지고, 그때는 실시간 값 대신 스냅샷 값을 보여준다.
 */
export function isSameWorkplaceDetail(detail, workplace) {
  const prefix = text(detail?.bizNoPrefix);
  if (prefix === "" || prefix !== text(workplace?.bizNoPrefix)) return false;
  return normalizeKeyPart(detail?.name) === normalizeKeyPart(workplace?.name);
}

/**
 * 목록 검색 결과에서 스냅샷의 사업장 행만 골라 월별 이력({seq, month})으로 정리한다.
 * 최근 기준월부터 정렬한다. 번호 앞자리가 같은 다른 사업장(같은 길의 이웃 포함)은
 * 버린다. 개명한 사업장은 남는 행이 없어 실시간 그래프를 그리지 않는다.
 */
export function pickSnapshotWorkplaceRows(items = [], workplace) {
  return items
    .filter((item) => item?.seq && item?.dataCreatedMonth && isSameWorkplaceDetail(item, workplace))
