...
export const BROKER_OFFICES_URL = OFFICES_URL;
const ADDRESS_COORD_URL = "https://api.vworld.kr/req/address";
...
// VWorld 주소 검색으로 도로명·지번 주소의 좌표를 찾는다(같은 키·도메인 사용).
// 도로명(type=ROAD)이 실패하면 지번(type=PARCEL)으로 다시 시도한다. 실패해도 null을
// 돌려 나머지 수집이 막히지 않게 한다.
export async function fetchAddressCoordinate(address, { key, domain, fetchImpl = globalThis.fetch, timeoutMs = DEFAULT_TIMEOUT_MS, maxRetries = 2, sleepImpl = sleep } = {}) {
  const query = clean(address);
  if (!query || !key || !domain) return null;
  const buildUrl = (type) => `${ADDRESS_COORD_URL}?${new URLSearchParams({
    service: "address", request: "getCoord", version: "2.0", crs: "EPSG:4326",
    address: query, type, key, domain
  })}`;
  for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
    for (const type of ["ROAD", "PARCEL"]) {
      try {
        const response = await fetchImpl(buildUrl(type), { signal: AbortSignal.timeout(timeoutMs) });
