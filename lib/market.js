...
/** 착한가격업소 명부와 상가업소명을 같은 키로 맞춘다. 괄호 안 지점·동 이름도 벗긴다. */
export function storeNameKey(value) {
  return String(value ?? "")
    .replace(/（[^）]*）|\([^)]*\)/g, "")
    .replace(/[\s()（）.,·\-_/]/g, "")
...
export function filterStores(stores, filters = {}) {
  ...
  return stores.filter((store) => {
    ...
    // 인허가 출처(모범음식점 등)가 새겨진 매장만 남긴다. 하나라도 걸리면 통과다.
    if (filters.sourceSlugs?.length && !filters.sourceSlugs.some((slug) => (store.sourceSlugs || []).includes(slug))) return false;
