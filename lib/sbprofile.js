...
/**
 * 집계 결과가 지난번과 같은지 본다. 수집 시각은 매번 달라지므로 비교에서 뺀다.
 * 같으면 파일을 새로 쓰지 않아 배치가 내용 없는 커밋을 남기지 않는다.
 */
export function isSameSnapshotData(previous, next) {
  if (!previous?.regions || !next?.regions) return false;
  return JSON.stringify(previous.regions) === JSON.stringify(next.regions)
    && JSON.stringify(previous.meta?.months ?? []) === JSON.stringify(next.meta?.months ?? []);
}
