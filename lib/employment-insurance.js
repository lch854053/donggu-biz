...
const CURRENT_INSURANCE_STATUSES = new Set(["계속", "일괄계속", "계속(일괄전환)"]);
const HISTORICAL_INSURANCE_STATUSES = new Set(["일괄유기", "유기"]);
...
/** 관리기록을 현재 가입·과거 종료·상태 미상으로 나눈다. */
export function insuranceStatusKind(row = {}) {
  const statuses = [row.employmentStatus, row.industrialStatus].filter(Boolean).map(String);
  if (statuses.some((status) => CURRENT_INSURANCE_STATUSES.has(status))) return "current";
  if (statuses.length && statuses.every((status) => HISTORICAL_INSURANCE_STATUSES.has(status))) return "historical";
  return "unknown";
}
...
function withManagementFields(row) {
  ...
  return {
    ...
    insuranceStatusKind: insuranceStatusKind(row)
  };
}
...
export function mergeEmploymentInsuranceRows(rows = []) {
  ...
  const grouped = [...groups.values()].map((sourceRows) => {
    ...
    const currentSourceRows = sourceRows.filter((row) => row.insuranceStatusKind === "current");
    const historicalSourceRows = sourceRows.filter((row) => row.insuranceStatusKind === "historical");
    const primaryRows = currentSourceRows.length ? currentSourceRows : sourceRows;
    const employmentRows = primaryRows.filter((row) => ["0", "2"].includes(row.insuranceType));
    const industrialRows = primaryRows.filter((row) => ["0", "1"].includes(row.insuranceType));
    ...
    return {
      ...
      insuranceStatusKind: currentSourceRows.length ? "current" : historicalSourceRows.length === sourceRows.length ? "historical" : "unknown",
