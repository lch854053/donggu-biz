...
function renderZoneOverview() {
  ...
  if (!mainBizZones.length) {
    $("zoneOverview").innerHTML = "<p>주요상권 데이터가 없습니다.</p>";
    return;
  }
  if (!zone) {
    $("zoneOverview").innerHTML = "<p>주요상권을 선택하면 경계 면적, 점포 수와 상위 업종 소분류를 확인할 수 있습니다.</p>";
    return;
  }
  ...
  $("zoneOverview").innerHTML = `<p class="zone-name">${escapeHtml(zone.properties.name)}</p>
    ...
    <p class="zone-category-title">상위 업종 소분류 10개</p>
    <div class="zone-categories">${summaryRows(countBy(visibleStores, "smallName"), visibleStores.length, 10)}</div>`;
}
