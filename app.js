...
async function loadDesignationMatches() {
  try {
    const response = await fetch("data/designation_matches_donggu.json", { cache: "no-cache" });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const payload = await response.json();
    designationIndex = { byName: payload.byName || {}, byBusinessNumber: payload.byBusinessNumber || {} };
  } catch (error) {
    // 지정기업 색인이 없어도 사업장 조회 자체는 막지 않는다.
    console.warn("[designations] 지정기업 색인을 불러오지 못했습니다:", error.message);
  }
}
...
function renderNpsTable() {
  ...
  body.innerHTML = rows.map((row, index) => {
    ...
    const designationBadges = (row.designations || [])
      .map((kind) => `<span class="badge badge-yellow">${escapeHtml(kind)}</span>`)
      .join(" ");
    ...
    <td class="seq">${offset + index + 1}</td>
    <td>${escapeHtml(name)}${designationBadges ? ` ${designationBadges}` : ""}</td>
